...
_yuitest_coverage["resize-proxy"] = {
    lines: {},
    functions: {},
    coveredLines: 0,
    calledLines: 0,
...
_yuitest_coverline("resize-proxy", 38);
function ResizeProxy() {
	_yuitest_coverfunc("resize-proxy", "ResizeProxy", 38);
_yuitest_coverline("resize-proxy", 39);
ResizeProxy.superclass.constructor.apply(this, arguments);
}

_yuitest_coverline("resize-proxy", 42);
Y.mix(ResizeProxy, {
	NAME: RESIZE_PROXY,
	...
	ATTRS: {
		...
		proxyNode: {
			setter: Y.one,
			valueFn: function() {
				_yuitest_coverfunc("resize-proxy", "valueFn", 57);
_yuitest_coverline("resize-proxy", 58);
...
_yuitest_coverline("resize-proxy", 64);
Y.extend(ResizeProxy, Y.Plugin.Base, {
	...
	initializer: function() {
		_yuitest_coverfunc("resize-proxy", "initializer", 73);
_yuitest_coverline("resize-proxy", 74);
var instance = this;

		_yuitest_coverline("resize-proxy", 76);
instance.afterHostEvent('resize:start', instance._afterResizeStart);
		_yuitest_coverline("resize-proxy", 77);
instance.beforeHostMethod('_resize', instance._beforeHostResize);
		_yuitest_coverline("resize-proxy", 78);
instance.afterHostMethod('_resizeEnd', instance._afterHostResizeEnd);
	},

	destructor: function() {
		_yuitest_coverfunc("resize-proxy", "destructor", 81);
_yuitest_coverline("resize-proxy", 82);
var instance = this;

		_yuitest_coverline("resize-proxy", 84);
instance.get(PROXY_NODE).remove(true);
	},

	_afterHostResizeEnd: function(event) {
		_yuitest_coverfunc("resize-proxy", "_afterHostResizeEnd", 87);
_yuitest_coverline("resize-proxy", 88);
var instance = this,
			drag = event.dragEvent.target;

		// reseting actXY from drag when drag end
		_yuitest_coverline("resize-proxy", 92);
drag.actXY = [];

		// if proxy is true, hide it on resize end
		_yuitest_coverline("resize-proxy", 95);
instance._syncProxyUI();

		_yuitest_coverline("resize-proxy", 97);
instance.get(PROXY_NODE).hide();
	},

	_afterResizeStart: function(event) {
		_yuitest_coverfunc("resize-proxy", "_afterResizeStart", 100);
_yuitest_coverline("resize-proxy", 101);
var instance = this;

		_yuitest_coverline("resize-proxy", 103);
instance._renderProxy();
	},

	_beforeHostResize: function(event) {
		_yuitest_coverfunc("resize-proxy", "_beforeHostResize", 106);
_yuitest_coverline("resize-proxy", 107);
var instance = this,
			host = this.get(HOST);

		_yuitest_coverline("resize-proxy", 110);
host._handleResizeAlignEvent(event.dragEvent);

		// if proxy is true _syncProxyUI instead of _syncUI
		_yuitest_coverline("resize-proxy", 113);
instance._syncProxyUI();

		_yuitest_coverline("resize-proxy", 115);
return new Y.Do.Prevent();
	...
	_renderProxy: function() {
		_yuitest_coverfunc("resize-proxy", "_renderProxy", 125);
_yuitest_coverline("resize-proxy", 126);
var instance = this,
			host = this.get(HOST),
			proxyNode = instance.get(PROXY_NODE);

		_yuitest_coverline("resize-proxy", 130);
if (!proxyNode.inDoc()) {
			_yuitest_coverline("resize-proxy", 131);
host.get(WRAPPER).get(PARENT_NODE).append(
	...
	_syncProxyUI: function() {
		_yuitest_coverfunc("resize-proxy", "_syncProxyUI", 144);
_yuitest_coverline("resize-proxy", 145);
var instance = this,
			host = this.get(HOST),
			info = host.info,
		...
		_yuitest_coverline("resize-proxy", 152);
proxyNode.show().setStyle(CURSOR, cursor);

		_yuitest_coverline("resize-proxy", 154);
host.delegate.dd.set(DRAG_CURSOR, cursor);

		_yuitest_coverline("resize-proxy", 156);
proxyNode.sizeTo(info.offsetWidth, info.offsetHeight);

		_yuitest_coverline("resize-proxy", 158);
proxyNode.setXY([ info.left, info.top ]);
	}
});

_yuitest_coverline("resize-proxy", 162);
Y.namespace('Plugin');
_yuitest_coverline("resize-proxy", 163);
