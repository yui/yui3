   ...
   */
    Dt = {
        formats: {
            ...
            G: function (d) {
                    var y = d.getFullYear(),
                        V = parseInt(Dt.formats.V(d), 10),
                        W = parseInt(Dt.formats.W(d), 10);
            ...
            j: function (d) {
                    var gmd_1 = new Date("" + d.getFullYear() + "/1/1 GMT"),
                        gmdate = new Date("" + d.getFullYear() + "/" + (d.getMonth()+1) + "/" + d.getDate() + " GMT"),
                        ms = gmdate - gmd_1,
            ...
            P: function (d, l) {return l.P[d.getHours() >= 12 ? 1 : 0 ];},
            s: function (d) {return parseInt(d.getTime()/1000, 10);},
            S: ["getSeconds", "0"],
    ...
    _expandAggregates: function (format) {
        var resources = Dt._resources,
            aggs = Dt.aggregates,
            regexp = new RegExp('%([' + Y.Object.keys(aggs).join('') + '])','g'),
            replace_aggs = function (m0, m1) {
                var f = aggs[m1];
                return (f === "locale" ? resources[m1] : f);
            };
        while(format.match(regexp)) {
            format = format.replace(regexp, replace_aggs);
        }
     ...
     * @param [oConfig] {Object | String} Object literal of configuration values.
     *      A string is a shorthand to `oConfig.format`.
     *
     *  <dl>
