...
var M = function(Y) {
    ...
    var _ = {
        ...
        insertBefore: function(node, child, refNode) {
            ...
            return refNode.parentNode.insertBefore(child, refNode); 
        },
    ...
    var applyStatic = function(f) {
        Y.Node[f] = function() {
            ...
            if (returnVal === undefined) {
                ...
                return Y.Node.getByNode(returnVal);

            } else if (returnVal.item && returnVal.length) { // wrap DOMNodeList
...
Selector.prototype = {
    ...
    operators: {
        ...
        '|=': function(attr, val) { return getRegExp('^' + val + '[-]?').test(attr); }, // Match start with value followed by optional hyphen '^=': function(attr, val) { return attr.indexOf(val) === 0; }, // Match starts with value
    ...
    query: function(selector, root, firstOnly) {
        ...
        Y.log('query: returning ' + result.length + ' nodes', 'info', 'Selector');
        return result;
    }
};
...
YAHOO.register("element", YAHOO.util.Node, {version: "@VERSION@", build: "@BUILD@"});
