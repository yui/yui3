...
_yuitest_coverage["resize-constrain"] = {
    lines: {},
    functions: {},
    coveredLines: 0,
    calledLines: 0,
...
var Lang = Y.Lang,
	...
	isNode = function(v) {
		_yuitest_coverfunc("resize-constrain", "isNode", 9);
_yuitest_coverline("resize-constrain", 10);
return (v instanceof Y.Node);
	},

	toNumber = function(num) {
		_yuitest_coverfunc("resize-constrain", "toNumber", 13);
_yuitest_coverline("resize-constrain", 14);
...
_yuitest_coverline("resize-constrain", 56);
function ResizeConstrained() {
	_yuitest_coverfunc("resize-constrain", "ResizeConstrained", 56);
_yuitest_coverline("resize-constrain", 57);
ResizeConstrained.superclass.constructor.apply(this, arguments);
}

_yuitest_coverline("resize-constrain", 60);
Y.mix(ResizeConstrained, {
	...
	ATTRS: {
        ...
        */
		constrain: {
			setter: function(v) {
				_yuitest_coverfunc("resize-constrain", "setter", 76);
_yuitest_coverline("resize-constrain", 77);
if (v && (isNode(v) || isString(v) || v.nodeType)) {
					_yuitest_coverline("resize-constrain", 78);
v = Y.one(v);
				}

				_yuitest_coverline("resize-constrain", 81);
return v;
			}
		},
...
_yuitest_coverline("resize-constrain", 169);
Y.extend(ResizeConstrained, Y.Plugin.Base, {
	...
	initializer: function() {
		_yuitest_coverfunc("resize-constrain", "initializer", 181);
_yuitest_coverline("resize-constrain", 182);
var instance = this,
			host = instance.get(HOST);

		_yuitest_coverline("resize-constrain", 185);
host.delegate.dd.plug(
		...
		_yuitest_coverline("resize-constrain", 193);
host.after('resize:align', Y.bind(instance._handleResizeAlignEvent, instance));
		_yuitest_coverline("resize-constrain", 194);
host.on('resize:start', Y.bind(instance._handleResizeStartEvent, instance));
	...
	_checkConstrain: function(axis, axisConstrain, offset) {
		_yuitest_coverfunc("resize-constrain", "_checkConstrain", 208);
_yuitest_coverline("resize-constrain", 209);
var instance = this,
			point1,
			point1Constrain,
			point2,
		...
		_yuitest_coverline("resize-constrain", 219);
if (region) {
			_yuitest_coverline("resize-constrain", 220);
point1 = info[axis] + info[offset];
			_yuitest_coverline("resize-constrain", 221);
point1Constrain = region[axisConstrain] - toNumber(constrainBorders[capitalize(BORDER, axisConstrain, WIDTH)]);

			_yuitest_coverline("resize-constrain", 223);
if (point1 >= point1Constrain) {
				_yuitest_coverline("resize-constrain", 224);
info[offset] -= (point1 - point1Constrain);
			}

			_yuitest_coverline("resize-constrain", 227);
point2 = info[axis];
			_yuitest_coverline("resize-constrain", 228);
point2Constrain = region[axis] + toNumber(constrainBorders[capitalize(BORDER, axis, WIDTH)]);

			_yuitest_coverline("resize-constrain", 230);
if (point2 <= point2Constrain) {
				_yuitest_coverline("resize-constrain", 231);
info[axis] += (point2Constrain - point2);
				_yuitest_coverline("resize-constrain", 232);
info[offset] -= (point2Constrain - point2);
	...
	_checkHeight: function() {
		_yuitest_coverfunc("resize-constrain", "_checkHeight", 244);
_yuitest_coverline("resize-constrain", 245);
var instance = this,
			host = instance.get(HOST),
			info = host.info,
		...
		_yuitest_coverline("resize-constrain", 251);
instance._checkConstrain(TOP, BOTTOM, OFFSET_HEIGHT);

		_yuitest_coverline("resize-constrain", 253);
if (info.offsetHeight > maxHeight) {
			_yuitest_coverline("resize-constrain", 254);
host._checkSize(OFFSET_HEIGHT, maxHeight);
		}

		_yuitest_coverline("resize-constrain", 257);
if (info.offsetHeight < minHeight) {
			_yuitest_coverline("resize-constrain", 258);
host._checkSize(OFFSET_HEIGHT, minHeight);
	...
	_checkRatio: function() {
		_yuitest_coverfunc("resize-constrain", "_checkRatio", 269);
_yuitest_coverline("resize-constrain", 270);
var instance = this,
			host = instance.get(HOST),
			info = host.info,
			...
			wRatio = function() {
				_yuitest_coverfunc("resize-constrain", "wRatio", 282);
_yuitest_coverline("resize-constrain", 283);
return (info.offsetWidth/oWidth);
			},
			hRatio = function() {
				_yuitest_coverfunc("resize-constrain", "hRatio", 285);
_yuitest_coverline("resize-constrain", 286);
...
isClosestToHeight = (bottomDiff < rightDiff);
		...
		_yuitest_coverline("resize-constrain", 321);
if (isClosestToHeight) {
			_yuitest_coverline("resize-constrain", 322);
info.offsetWidth = oWidth*hRatio();
			_yuitest_coverline("resize-constrain", 323);
instance._checkWidth();
			_yuitest_coverline("resize-constrain", 324);
info.offsetHeight = oHeight*wRatio();
		}
		else {
			_yuitest_coverline("resize-constrain", 327);
info.offsetHeight = oHeight*wRatio();
			_yuitest_coverline("resize-constrain", 328);
instance._checkHeight();
			_yuitest_coverline("resize-constrain", 329);
info.offsetWidth = oWidth*hRatio();
		}
		...
		_yuitest_coverline("resize-constrain", 334);
if (host.changeTopHandles) {
			_yuitest_coverline("resize-constrain", 335);
info.top = oTop + (oHeight - info.offsetHeight);
		...
		_yuitest_coverline("resize-constrain", 340);
if (host.changeLeftHandles) {
			_yuitest_coverline("resize-constrain", 341);
info.left = oLeft + (oWidth - info.offsetWidth);
		}

		// rounding values to avoid pixel jumpings
		_yuitest_coverline("resize-constrain", 345);
Y.each(info, function(value, key) {
			_yuitest_coverfunc("resize-constrain", "(anonymous 2)", 345);
_yuitest_coverline("resize-constrain", 346);
if (isNumber(value)) {
				_yuitest_coverline("resize-constrain", 347);
info[key] = Math.round(value);
	...
	_checkRegion: function() {
		_yuitest_coverfunc("resize-constrain", "_checkRegion", 359);
_yuitest_coverline("resize-constrain", 360);
var instance = this,
			host = instance.get(HOST),
			region = instance._getConstrainRegion();

		_yuitest_coverline("resize-constrain", 364);
return Y.DOM.inRegion(null, region, true, host.info);
	...
	_checkWidth: function() {
		_yuitest_coverfunc("resize-constrain", "_checkWidth", 374);
_yuitest_coverline("resize-constrain", 375);
var instance = this,
			host = instance.get(HOST),
			info = host.info,
		...
		_yuitest_coverline("resize-constrain", 381);
instance._checkConstrain(LEFT, RIGHT, OFFSET_WIDTH);

		_yuitest_coverline("resize-constrain", 383);
if (info.offsetWidth < minWidth) {
			_yuitest_coverline("resize-constrain", 384);
host._checkSize(OFFSET_WIDTH, minWidth);
		}

		_yuitest_coverline("resize-constrain", 387);
if (info.offsetWidth > maxWidth) {
			_yuitest_coverline("resize-constrain", 388);
host._checkSize(OFFSET_WIDTH, maxWidth);
	...
	_getConstrainRegion: function() {
		_yuitest_coverfunc("resize-constrain", "_getConstrainRegion", 400);
_yuitest_coverline("resize-constrain", 401);
var instance = this,
			host = instance.get(HOST),
			node = host.get(NODE),
		...
		_yuitest_coverline("resize-constrain", 407);
if (constrain) {
			_yuitest_coverline("resize-constrain", 408);
if (constrain == VIEW) {
				_yuitest_coverline("resize-constrain", 409);
region = node.get(VIEWPORT_REGION);
			}
			else {_yuitest_coverline("resize-constrain", 411);
if (isNode(constrain)) {
				_yuitest_coverline("resize-constrain", 412);
region = constrain.get(REGION);
			}
			else {
				_yuitest_coverline("resize-constrain", 415);
region = constrain;
			}}
		}

		_yuitest_coverline("resize-constrain", 419);
return region;
	},

	_handleResizeAlignEvent: function(event) {
		_yuitest_coverfunc("resize-constrain", "_handleResizeAlignEvent", 422);
_yuitest_coverline("resize-constrain", 423);
var instance = this,
			host = instance.get(HOST);

		// check the max/min height and locking top when these values are reach
		_yuitest_coverline("resize-constrain", 427);
instance._checkHeight();

		// check the max/min width and locking left when these values are reach
		_yuitest_coverline("resize-constrain", 430);
instance._checkWidth();

		// calculating the ratio, for proportionally resizing
		_yuitest_coverline("resize-constrain", 433);
if (instance.get(PRESEVE_RATIO)) {
			_yuitest_coverline("resize-constrain", 434);
instance._checkRatio();
		}

		_yuitest_coverline("resize-constrain", 437);
if (instance.get(CONSTRAIN) && !instance._checkRegion()) {
			_yuitest_coverline("resize-constrain", 438);
host.info = host.lastInfo;
		}
	},

	_handleResizeStartEvent: function(event) {
		_yuitest_coverfunc("resize-constrain", "_handleResizeStartEvent", 442);
_yuitest_coverline("resize-constrain", 443);
var instance = this,
			constrain = instance.get(CONSTRAIN),
			host = instance.get(HOST);

		_yuitest_coverline("resize-constrain", 447);
instance.constrainSurrounding = host._getBoxSurroundingInfo(constrain);
	}
});

_yuitest_coverline("resize-constrain", 451);
Y.namespace('Plugin');
_yuitest_coverline("resize-constrain", 452);
