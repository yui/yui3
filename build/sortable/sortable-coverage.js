...
YUI.add('sortable', function (Y, NAME) {
    ...
    _yuitest_coverfunc("sortable", "(anonymous 1)", 1);
_yuitest_coverline("sortable", 16);
var Sortable = function(o) {
        _yuitest_coverfunc("sortable", "Sortable", 16);
_yuitest_coverline("sortable", 17);
Sortable.superclass.constructor.apply(this, arguments);
    ...
    _yuitest_coverline("sortable", 30);
Y.extend(Sortable, Y.Base, {
        ...
        */
        ...
        initializer: function() {
            _yuitest_coverfunc("sortable", "initializer", 43);
_yuitest_coverline("sortable", 44);
var id = 'sortable-' + Y.guid(),
            ...
            _yuitest_coverline("sortable", 55);
if (this.get('handles')) {
                _yuitest_coverline("sortable", 56);
delConfig.handles = this.get('handles');
            }
            _yuitest_coverline("sortable", 58);
del = new Y.DD.Delegate(delConfig);

            _yuitest_coverline("sortable", 60);
this.set(ID, id);

            _yuitest_coverline("sortable", 62);
del.dd.plug(Y.Plugin.DDProxy, {
                moveOnEnd: false,
                cloneNode: true
            });

            _yuitest_coverline("sortable", 67);
this.drop =  new Y.DD.Drop({
                node: this.get(CONT),
                bubbleTarget: del,
                groups: del.dd.get('groups')
            });
            _yuitest_coverline("sortable", 72);
this.drop.on('drop:over', Y.bind(this._onDropOver, this));
            
            _yuitest_coverline("sortable", 74);
del.on({
            ...
            _yuitest_coverline("sortable", 81);
this.delegate = del;
            _yuitest_coverline("sortable", 82);
Sortable.reg(this);
        },
        _up: null,
        _y: null,
        _onDrag: function(e) {
            _yuitest_coverfunc("sortable", "_onDrag", 86);
_yuitest_coverline("sortable", 87);
if (e.pageY < this._y) {
                _yuitest_coverline("sortable", 88);
this._up = true; 
            } else {_yuitest_coverline("sortable", 89);
if (e.pageY > this._y) { 
                _yuitest_coverline("sortable", 90);
this._up = false; 
            }} 

            _yuitest_coverline("sortable", 93);
this._y = e.pageY;
        },
        /**
        * @private
        ...
        */
        _onDropOver: function(e) {
            _yuitest_coverfunc("sortable", "_onDropOver", 101);
_yuitest_coverline("sortable", 102);
if (!e.drop.get(NODE).test(this.get(NODES))) {
                _yuitest_coverline("sortable", 103);
e.drop.get(NODE).append(e.drag.get(NODE));
        ...
        */
        _onDragOver: function(e) {
            _yuitest_coverfunc("sortable", "_onDragOver", 112);
_yuitest_coverline("sortable", 113);
if (!e.drop.get(NODE).test(this.get(NODES))) {
                _yuitest_coverline("sortable", 114);
return;
            }
            _yuitest_coverline("sortable", 116);
if (e.drag.get(NODE) == e.drop.get(NODE)) {
                _yuitest_coverline("sortable", 117);
return;
            }
            // is drop a child of drag?
            _yuitest_coverline("sortable", 120);
if (e.drag.get(NODE).contains(e.drop.get(NODE))) {
                _yuitest_coverline("sortable", 121);
return;
            }
            _yuitest_coverline("sortable", 123);
var same = false, dir, oldNode, newNode, dropsort, dropNode,
                moveType = this.get('moveType').toLowerCase();

            _yuitest_coverline("sortable", 126);
if (e.drag.get(NODE).get(PARENT_NODE).contains(e.drop.get(NODE))) {
                _yuitest_coverline("sortable", 127);
same = true;
            }
            _yuitest_coverline("sortable", 129);
if (same && moveType == 'move') {
                _yuitest_coverline("sortable", 130);
moveType = 'insert';
            }
            _yuitest_coverline("sortable", 132);
switch (moveType) {
                case 'insert':
                    _yuitest_coverline("sortable", 134);
dir = ((this._up) ? 'before' : 'after');
                    _yuitest_coverline("sortable", 135);
dropNode = e.drop.get(NODE);
                    _yuitest_coverline("sortable", 136);
if (Y.Sortable._test(dropNode, this.get(CONT))) {
                        _yuitest_coverline("sortable", 137);
dropNode.append(e.drag.get(NODE));
                    } else {
                        _yuitest_coverline("sortable", 139);
dropNode.insert(e.drag.get(NODE), dir);
                    }
                    _yuitest_coverline("sortable", 141);
break;
                case 'swap':
                    _yuitest_coverline("sortable", 143);
Y.DD.DDM.swapNode(e.drag, e.drop);
                    _yuitest_coverline("sortable", 144);
break;
                case 'move':
                case 'copy':
                    _yuitest_coverline("sortable", 147);
dropsort = Y.Sortable.getSortable(e.drop.get(NODE).get(PARENT_NODE));

                    _yuitest_coverline("sortable", 149);
if (!dropsort) {
                        _yuitest_coverline("sortable", 150);
return;
                    }
                    
                    _yuitest_coverline("sortable", 153);
Y.DD.DDM.getDrop(e.drag.get(NODE)).addToGroup(dropsort.get(ID));

                    //Same List
                    _yuitest_coverline("sortable", 156);
if (same) {
                        _yuitest_coverline("sortable", 157);
Y.DD.DDM.swapNode(e.drag, e.drop);
                    } else {
                        _yuitest_coverline("sortable", 159);
if (this.get('moveType') == 'copy') {
                            //New List
                            _yuitest_coverline("sortable", 161);
oldNode = e.drag.get(NODE);
                            _yuitest_coverline("sortable", 162);
newNode = oldNode.cloneNode(true);

                            _yuitest_coverline("sortable", 164);
newNode.set(ID, '');
                            _yuitest_coverline("sortable", 165);
e.drag.set(NODE, newNode);
                            _yuitest_coverline("sortable", 166);
dropsort.delegate.createDrop(newNode, [dropsort.get(ID)]);
                            _yuitest_coverline("sortable", 167);
oldNode.setStyles({
                                top: '',
                                left: ''
                            });
                        }
                        _yuitest_coverline("sortable", 172);
e.drop.get(NODE).insert(e.drag.get(NODE), 'before');
                    }
                    _yuitest_coverline("sortable", 174);
break;
            }

            _yuitest_coverline("sortable", 177);
this.fire(moveType, { same: same, drag: e.drag, drop: e.drop });
            _yuitest_coverline("sortable", 178);
this.fire('moved', { same: same, drag: e.drag, drop: e.drop });
        ...
        */
        _onDragStart: function(e) {
            _yuitest_coverfunc("sortable", "_onDragStart", 186);
_yuitest_coverline("sortable", 187);
var del = this.delegate,
                lastNode = del.get('lastNode');
            _yuitest_coverline("sortable", 189);
if (lastNode && lastNode.getDOMNode()) {
                _yuitest_coverline("sortable", 190);
lastNode.setStyle(ZINDEX, '');
            }
            _yuitest_coverline("sortable", 192);
del.get(this.get(OPACITY_NODE)).setStyle(OPACITY, this.get(OPACITY));
            _yuitest_coverline("sortable", 193);
del.get(CURRENT_NODE).setStyle(ZINDEX, '999');
        ...
        */
        _onDragEnd: function(e) {
            _yuitest_coverfunc("sortable", "_onDragEnd", 201);
_yuitest_coverline("sortable", 202);
this.delegate.get(this.get(OPACITY_NODE)).setStyle(OPACITY, 1);
            _yuitest_coverline("sortable", 203);
this.delegate.get(CURRENT_NODE).setStyles({
                top: '',
                left: ''
            });
            _yuitest_coverline("sortable", 207);
this.sync();
        },
        /**
        * @method plug
        ...
        */
        plug: function(cls, config) {
            ...
            _yuitest_coverfunc("sortable", "plug", 216);
_yuitest_coverline("sortable", 218);
if (cls && cls.NAME.substring(0, 4).toLowerCase() === 'sort') {
                _yuitest_coverline("sortable", 219);
this.constructor.superclass.plug.call(this, cls, config);
            } else {
                _yuitest_coverline("sortable", 221);
this.delegate.dd.plug(cls, config);
            }
            _yuitest_coverline("sortable", 223);
return this;
        },
        /**
        * @method sync
        ...
        */
        sync: function() {
            _yuitest_coverfunc("sortable", "sync", 230);
_yuitest_coverline("sortable", 231);
this.delegate.syncTargets();
            _yuitest_coverline("sortable", 232);
return this;
        },
        destructor: function() {
            _yuitest_coverfunc("sortable", "destructor", 234);
_yuitest_coverline("sortable", 235);
this.drop.destroy();
            _yuitest_coverline("sortable", 236);
this.delegate.destroy();
            _yuitest_coverline("sortable", 237);
Sortable.unreg(this);
        },
        /**
        * @method join
        ...
        */
        join: function(sel, type) {
            _yuitest_coverfunc("sortable", "join", 252);
_yuitest_coverline("sortable", 253);
if (!(sel instanceof Y.Sortable)) {
                _yuitest_coverline("sortable", 254);
Y.error('Sortable: join needs a Sortable Instance');
                _yuitest_coverline("sortable", 255);
return this;
            }
            _yuitest_coverline("sortable", 257);
if (!type) {
                _yuitest_coverline("sortable", 258);
type = 'full';
            }
            _yuitest_coverline("sortable", 260);
type = type.toLowerCase();
            _yuitest_coverline("sortable", 261);
var method = '_join_' + type;

            _yuitest_coverline("sortable", 263);
if (this[method]) {
                _yuitest_coverline("sortable", 264);
this[method](sel);
            }
            
            _yuitest_coverline("sortable", 267);
return this;
        },
        /**
        * @private
        ...
        */
        _join_none: function(sel) {
            _yuitest_coverfunc("sortable", "_join_none", 275);
_yuitest_coverline("sortable", 276);
this.delegate.dd.removeFromGroup(sel.get(ID));
            _yuitest_coverline("sortable", 277);
sel.delegate.dd.removeFromGroup(this.get(ID));
        ...
        */
        _join_full: function(sel) {
            _yuitest_coverfunc("sortable", "_join_full", 285);
_yuitest_coverline("sortable", 286);
this.delegate.dd.addToGroup(sel.get(ID));
            _yuitest_coverline("sortable", 287);
sel.delegate.dd.addToGroup(this.get(ID));
        ...
        */
        _join_outer: function(sel) {
            _yuitest_coverfunc("sortable", "_join_outer", 295);
_yuitest_coverline("sortable", 296);
this.delegate.dd.addToGroup(sel.get(ID));
        },
        /**
        * @private
        ...
        */
        _join_inner: function(sel) {
            _yuitest_coverfunc("sortable", "_join_inner", 304);
_yuitest_coverline("sortable", 305);
sel.delegate.dd.addToGroup(this.get(ID));
        },
        /**
        ...
        */
        getOrdering: function(callback) {
            _yuitest_coverfunc("sortable", "getOrdering", 313);
_yuitest_coverline("sortable", 314);
var ordering = [];

            _yuitest_coverline("sortable", 316);
if (!Y.Lang.isFunction(callback)) {
                _yuitest_coverline("sortable", 317);
callback = function (node) {
                    _yuitest_coverfunc("sortable", "callback", 317);
_yuitest_coverline("sortable", 318);
return node;
                };
            }

            _yuitest_coverline("sortable", 322);
Y.one(this.get(CONT)).all(this.get(NODES)).each(function(node) {
                _yuitest_coverfunc("sortable", "(anonymous 2)", 322);
_yuitest_coverline("sortable", 323);
ordering.push(callback(node));
            });
            _yuitest_coverline("sortable", 325);
return ordering;
       }
    }, {
        NAME: 'sortable',
        ...
        */
        _test: function(node, test) {
            _yuitest_coverfunc("sortable", "_test", 410);
_yuitest_coverline("sortable", 411);
var ret;
            _yuitest_coverline("sortable", 412);
if (test instanceof Y.Node) {
                _yuitest_coverline("sortable", 413);
ret = (test === node);
            } else {
                _yuitest_coverline("sortable", 415);
ret = node.test(test);
            }
            _yuitest_coverline("sortable", 417);
return ret;
        },
        /**
        * @static
        ...
        */
        getSortable: function(node) {
            _yuitest_coverfunc("sortable", "getSortable", 425);
_yuitest_coverline("sortable", 426);
var s = null;
            _yuitest_coverline("sortable", 427);
node = Y.one(node);
            _yuitest_coverline("sortable", 428);
Y.each(Y.Sortable._sortables, function(v) {
                _yuitest_coverfunc("sortable", "(anonymous 3)", 428);
_yuitest_coverline("sortable", 429);
if (Y.Sortable._test(node, v.get(CONT))) {
                    _yuitest_coverline("sortable", 430);
s = v;
                }
            });
            _yuitest_coverline("sortable", 433);
return s;
        },
        /**
        * @static
        ...
        */
        reg: function(s) {
            _yuitest_coverfunc("sortable", "reg", 441);
_yuitest_coverline("sortable", 442);
Y.Sortable._sortables.push(s);
        ...
        */
        unreg: function(s) {
            _yuitest_coverfunc("sortable", "unreg", 450);
_yuitest_coverline("sortable", 451);
Y.each(Y.Sortable._sortables, function(v, k) {
                _yuitest_coverfunc("sortable", "(anonymous 4)", 451);
_yuitest_coverline("sortable", 452);
if (v === s) {
                    _yuitest_coverline("sortable", 453);
Y.Sortable._sortables[k] = null;
                    _yuitest_coverline("sortable", 454);
delete Sortable._sortables[k];
    ...
    _yuitest_coverline("sortable", 460);
Y.Sortable = Sortable;
