...
_yuitest_coverfunc("yui-later", "(anonymous 1)", 1);
_yuitest_coverline("yui-later", 10);
var NO_ARGS = [];
 ...
 */
_yuitest_coverline("yui-later", 37);
Y.later = function(when, o, fn, data, periodic) {
    _yuitest_coverfunc("yui-later", "later", 37);
_yuitest_coverline("yui-later", 38);
when = when || 0;
    _yuitest_coverline("yui-later", 39);
data = (!Y.Lang.isUndefined(data)) ? Y.Array(data) : NO_ARGS;
    _yuitest_coverline("yui-later", 40);
o = o || Y.config.win || Y;

    _yuitest_coverline("yui-later", 42);
var cancelled = false,
        ...
        wrapper = function() {
            ...
            // an extra hoop.
            _yuitest_coverfunc("yui-later", "wrapper", 44);
_yuitest_coverline("yui-later", 49);
if (!cancelled) {
                _yuitest_coverline("yui-later", 50);
if (!method.apply) {
                    _yuitest_coverline("yui-later", 51);
method(data[0], data[1], data[2], data[3]);
                } else {
                    _yuitest_coverline("yui-later", 53);
method.apply(o, data || NO_ARGS);
    ...
    _yuitest_coverline("yui-later", 59);
return {
        id: id,
        interval: periodic,
        cancel: function() {
            _yuitest_coverfunc("yui-later", "cancel", 62);
_yuitest_coverline("yui-later", 63);
cancelled = true;
            _yuitest_coverline("yui-later", 64);
if (this.interval) {
                _yuitest_coverline("yui-later", 65);
clearInterval(id);
            } else {
                _yuitest_coverline("yui-later", 67);
clearTimeout(id);
            }
        }
    };
...
_yuitest_coverline("yui-later", 73);
