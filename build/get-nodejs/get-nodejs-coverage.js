...
YUI.add('get-nodejs', function (Y, NAME) {
    ...
    */
    ...
    _yuitest_coverfunc("get-nodejs", "(anonymous 1)", 1);
_yuitest_coverline("get-nodejs", 9);
var path = require('path'),
    ...
    _yuitest_coverline("get-nodejs", 16);
Y.Get = function() {
    };

    //Setup the default config base path
    _yuitest_coverline("get-nodejs", 20);
Y.config.base = path.join(__dirname, '../');

    _yuitest_coverline("get-nodejs", 22);
YUI.require = require;
    _yuitest_coverline("get-nodejs", 23);
YUI.process = process;
    ...
    */
    _yuitest_coverline("get-nodejs", 32);
var escapeWinPath = function(p) {
        _yuitest_coverfunc("get-nodejs", "escapeWinPath", 32);
_yuitest_coverline("get-nodejs", 33);
return p.replace(/\\/g, '\\\\');
    };
    ...
    */
    ...
    _yuitest_coverline("get-nodejs", 48);
Y.Get._exec = function(data, url, cb) {
        _yuitest_coverfunc("get-nodejs", "_exec", 48);
_yuitest_coverline("get-nodejs", 49);
var dirName = escapeWinPath(path.dirname(url));
        _yuitest_coverline("get-nodejs", 50);
var fileName = escapeWinPath(url);

        _yuitest_coverline("get-nodejs", 52);
if (dirName.match(/^https?:\/\//)) {
            _yuitest_coverline("get-nodejs", 53);
dirName = '.';
            _yuitest_coverline("get-nodejs", 54);
fileName = 'remoteResource';
        }

        _yuitest_coverline("get-nodejs", 57);
var mod = "(function(YUI) { var __dirname = '" + dirName + "'; "+
        ...
        _yuitest_coverline("get-nodejs", 67);
var script = vm.createScript(mod, url);
        _yuitest_coverline("get-nodejs", 68);
var fn = script.runInThisContext(mod);
        _yuitest_coverline("get-nodejs", 69);
YUI = fn(YUI);
        _yuitest_coverline("get-nodejs", 70);
cb(null, url);
    };
    ...
    */
    _yuitest_coverline("get-nodejs", 81);
Y.Get._include = function(url, cb) {
        _yuitest_coverfunc("get-nodejs", "_include", 81);
_yuitest_coverline("get-nodejs", 82);
var self = this;

        _yuitest_coverline("get-nodejs", 84);
if (url.match(/^https?:\/\//)) {
            _yuitest_coverline("get-nodejs", 85);
var cfg = {
                url: url,
                timeout: self.timeout
            };
            _yuitest_coverline("get-nodejs", 89);
request(cfg, function (err, response, body) {
                _yuitest_coverfunc("get-nodejs", "(anonymous 2)", 89);
_yuitest_coverline("get-nodejs", 90);
if (err) {
                    _yuitest_coverline("get-nodejs", 91);
cb(err, url);
                } else {
                    _yuitest_coverline("get-nodejs", 93);
Y.Get._exec(body, url, cb);
                }
            });
        } else {
            _yuitest_coverline("get-nodejs", 97);
if (Y.config.useSync) {
                //Needs to be in useSync
                _yuitest_coverline("get-nodejs", 99);
if (existsSync(url)) {
                    _yuitest_coverline("get-nodejs", 100);
var mod = fs.readFileSync(url,'utf8');
                    _yuitest_coverline("get-nodejs", 101);
Y.Get._exec(mod, url, cb);
                } else {
                    _yuitest_coverline("get-nodejs", 103);
cb('Path does not exist: ' + url, url);
                }
            } else {
                _yuitest_coverline("get-nodejs", 106);
fs.readFile(url, 'utf8', function(err, mod) {
                    _yuitest_coverfunc("get-nodejs", "(anonymous 3)", 106);
_yuitest_coverline("get-nodejs", 107);
if (err) {
                        _yuitest_coverline("get-nodejs", 108);
cb(err, url);
                    } else {
                        _yuitest_coverline("get-nodejs", 110);
Y.Get._exec(mod, url, cb);
                    }
                });
            }
    ...
    _yuitest_coverline("get-nodejs", 119);
var end = function(cb, msg, result) {
        _yuitest_coverfunc("get-nodejs", "end", 119);
_yuitest_coverline("get-nodejs", 120);
if (Y.Lang.isFunction(cb.onEnd)) {
            _yuitest_coverline("get-nodejs", 121);
cb.onEnd.call(Y, msg, result);
        }
    }, pass = function(cb) {
        _yuitest_coverfunc("get-nodejs", "pass", 123);
_yuitest_coverline("get-nodejs", 124);
if (Y.Lang.isFunction(cb.onSuccess)) {
            _yuitest_coverline("get-nodejs", 125);
cb.onSuccess.call(Y, cb);
        }
        _yuitest_coverline("get-nodejs", 127);
end(cb, 'success', 'success');
    }, fail = function(cb, er) {
        _yuitest_coverfunc("get-nodejs", "fail", 128);
_yuitest_coverline("get-nodejs", 129);
er.errors = [er];
        _yuitest_coverline("get-nodejs", 130);
if (Y.Lang.isFunction(cb.onFailure)) {
            _yuitest_coverline("get-nodejs", 131);
cb.onFailure.call(Y, er, cb);
        }
        _yuitest_coverline("get-nodejs", 133);
end(cb, er, 'fail');
    };
    ...
    */
    _yuitest_coverline("get-nodejs", 143);
Y.Get.js = function(s, options) {
        _yuitest_coverfunc("get-nodejs", "js", 143);
_yuitest_coverline("get-nodejs", 144);
var A = Y.Array,
            self = this,
            urls = A(s), url, i, l = urls.length, c= 0,
            check = function() {
                _yuitest_coverfunc("get-nodejs", "check", 147);
_yuitest_coverline("get-nodejs", 148);
if (c === l) {
                    _yuitest_coverline("get-nodejs", 149);
pass(options);
                }
            };
        ...
        _yuitest_coverline("get-nodejs", 155);
for (i=0; i<l; i++) {
            _yuitest_coverline("get-nodejs", 156);
url = urls[i];
            _yuitest_coverline("get-nodejs", 157);
if (Y.Lang.isObject(url)) {
                _yuitest_coverline("get-nodejs", 158);
url = url.url;
            }

            _yuitest_coverline("get-nodejs", 161);
url = url.replace(/'/g, '%27');
            _yuitest_coverline("get-nodejs", 162);
Y.Get._include(url, function(err, url) {
                _yuitest_coverfunc("get-nodejs", "(anonymous 4)", 162);
_yuitest_coverline("get-nodejs", 163);
if (!Y.config) {
                    _yuitest_coverline("get-nodejs", 164);
Y.config = {
                        debug: true
                    };
                }
                _yuitest_coverline("get-nodejs", 168);
if (options.onProgress) {
                    _yuitest_coverline("get-nodejs", 169);
options.onProgress.call(options.context || Y, url);
                }
                _yuitest_coverline("get-nodejs", 171);
if (err) {
                    _yuitest_coverline("get-nodejs", 172);
fail(options, err);
                } else {
                    _yuitest_coverline("get-nodejs", 174);
c++;
                    _yuitest_coverline("get-nodejs", 175);
check();
                }
            });
        }
    ...
    * @method script
    */
    _yuitest_coverline("get-nodejs", 185);
Y.Get.script = Y.Get.js;
    
    //Place holder for SS Dom access
    _yuitest_coverline("get-nodejs", 188);
Y.Get.css = function(s, cb) {
        _yuitest_coverfunc("get-nodejs", "css", 188);
_yuitest_coverline("get-nodejs", 189);
pass(cb);
    };
