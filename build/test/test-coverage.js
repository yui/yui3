...
_yuitest_coverfunc("test", "(anonymous 1)", 1);
_yuitest_coverline("test", 16);
if (YUI.YUITest) {
    _yuitest_coverline("test", 17);
Y.Test = YUI.YUITest;
} else { //Ends after the YUITest definitions

    //Make this global for back compat
    _yuitest_coverline("test", 21);
YUITest = {
        version: "@VERSION@",
        guid: function(pre) {
            _yuitest_coverfunc("test", "guid", 23);
_yuitest_coverline("test", 24);
return Y.guid(pre);
        }
    };

_yuitest_coverline("test", 28);
Y.namespace('Test');


//Using internal YUI methods here
_yuitest_coverline("test", 32);
YUITest.Object = Y.Object;
_yuitest_coverline("test", 33);
YUITest.Array = Y.Array;
_yuitest_coverline("test", 34);
YUITest.Util = {
    mix: Y.mix,
    JSON: Y.JSON
};
...
YUITest.EventTarget = function(){
     ...
     * @static
     */
    _yuitest_coverfunc("test", "EventTarget", 46);
_yuitest_coverline("test", 55);
this._handlers = {};

};
    
_yuitest_coverline("test", 59);
YUITest.EventTarget.prototype = {
    ...
    attach: function(type, listener){
        _yuitest_coverfunc("test", "attach", 75);
_yuitest_coverline("test", 76);
if (typeof this._handlers[type] == "undefined"){
            _yuitest_coverline("test", 77);
this._handlers[type] = [];
        }

        _yuitest_coverline("test", 80);
this._handlers[type].push(listener);
    ...
    subscribe: function(type, listener){
        _yuitest_coverfunc("test", "subscribe", 91);
_yuitest_coverline("test", 92);
this.attach.apply(this, arguments);
     ...
     */    
    fire: function(event){
        _yuitest_coverfunc("test", "fire", 102);
_yuitest_coverline("test", 103);
if (typeof event == "string"){
            _yuitest_coverline("test", 104);
event = { type: event };
        }
        _yuitest_coverline("test", 106);
if (!event.target){
            _yuitest_coverline("test", 107);
event.target = this;
        }
        
        _yuitest_coverline("test", 110);
if (!event.type){
            _yuitest_coverline("test", 111);
throw new Error("Event object missing 'type' property.");
        }
        
        _yuitest_coverline("test", 114);
if (this._handlers[event.type] instanceof Array){
            _yuitest_coverline("test", 115);
var handlers = this._handlers[event.type];
            _yuitest_coverline("test", 116);
for (var i=0, len=handlers.length; i < len; i++){
                _yuitest_coverline("test", 117);
handlers[i].call(this, event);
    ...
    detach: function(type, listener){
        _yuitest_coverfunc("test", "detach", 129);
_yuitest_coverline("test", 130);
if (this._handlers[type] instanceof Array){
            _yuitest_coverline("test", 131);
var handlers = this._handlers[type];
            _yuitest_coverline("test", 132);
for (var i=0, len=handlers.length; i < len; i++){
                _yuitest_coverline("test", 133);
if (handlers[i] === listener){
                    _yuitest_coverline("test", 134);
handlers.splice(i, 1);
                    _yuitest_coverline("test", 135);
break;
                }
            }
        }            
    ...
    unsubscribe: function(type, listener){
        _yuitest_coverfunc("test", "unsubscribe", 149);
_yuitest_coverline("test", 150);
...
YUITest.TestSuite = function (data) {
     ...
     * @property name
     */
    _yuitest_coverfunc("test", "TestSuite", 165);
_yuitest_coverline("test", 172);
this.name = "";
     ...
     * @private
     */
    _yuitest_coverline("test", 180);
this.items = [];

    //initialize the properties
    _yuitest_coverline("test", 183);
if (typeof data == "string"){
        _yuitest_coverline("test", 184);
this.name = data;
    } else {_yuitest_coverline("test", 185);
if (data instanceof Object){
        _yuitest_coverline("test", 186);
for (var prop in data){
            _yuitest_coverline("test", 187);
if (data.hasOwnProperty(prop)){
                _yuitest_coverline("test", 188);
this[prop] = data[prop];
            }
        }
    }}
    ...
    //double-check name
    _yuitest_coverline("test", 194);
if (this.name === "" || !this.name) {
        _yuitest_coverline("test", 195);
this.name = YUITest.guid("testSuite_");
    }

};
    
_yuitest_coverline("test", 200);
YUITest.TestSuite.prototype = {
    ...
    add : function (testObject) {
        _yuitest_coverfunc("test", "add", 211);
_yuitest_coverline("test", 212);
if (testObject instanceof YUITest.TestSuite || testObject instanceof YUITest.TestCase) {
            _yuitest_coverline("test", 213);
this.items.push(testObject);
        }
        _yuitest_coverline("test", 215);
return this;
    },
...
YUITest.TestCase = function (template) {
     ...
     */
    _yuitest_coverfunc("test", "TestCase", 251);
_yuitest_coverline("test", 258);
this._should = {};
    
    //copy over all properties from the template to this object
    _yuitest_coverline("test", 261);
for (var prop in template) {
        _yuitest_coverline("test", 262);
this[prop] = template[prop];
    }    
    
    //check for a valid name
    _yuitest_coverline("test", 266);
if (typeof this.name != "string") {
        _yuitest_coverline("test", 267);
...
YUITest.TestCase.prototype = {  
     ...
     */
    callback: function(){
        _yuitest_coverfunc("test", "callback", 285);
_yuitest_coverline("test", 286);
return YUITest.TestRunner.callback.apply(YUITest.TestRunner,arguments);
     ...
     */
    resume : function (segment) {
        _yuitest_coverfunc("test", "resume", 296);
_yuitest_coverline("test", 297);
YUITest.TestRunner.resume(segment);
    ...
    wait : function (segment, delay){
        ...
        _yuitest_coverfunc("test", "wait", 310);
_yuitest_coverline("test", 312);
var actualDelay = (typeof segment == "number" ? segment : delay);
        _yuitest_coverline("test", 313);
actualDelay = (typeof actualDelay == "number" ? actualDelay : 10000);
    
		_yuitest_coverline("test", 315);
if (typeof segment == "function"){
            _yuitest_coverline("test", 316);
throw new YUITest.Wait(segment, actualDelay);
        } else {
            _yuitest_coverline("test", 318);
throw new YUITest.Wait(function(){
                _yuitest_coverfunc("test", "(anonymous 2)", 318);
_yuitest_coverline("test", 319);
YUITest.Assert.fail("Timeout: wait() called but resume() never called.");
    ...
    assert : function (condition, message){
        _yuitest_coverfunc("test", "assert", 335);
_yuitest_coverline("test", 336);
YUITest.Assert._increment();
        _yuitest_coverline("test", 337);
if (!condition){
            _yuitest_coverline("test", 338);
throw new YUITest.AssertionError(YUITest.Assert._formatMessage(message, "Assertion failed."));
    ...
    fail: function (message) {    
        _yuitest_coverfunc("test", "fail", 347);
_yuitest_coverline("test", 348);
...
YUITest.TestFormat = function(){
     ...
     */
    _yuitest_coverfunc("test", "TestFormat", 398);
_yuitest_coverline("test", 406);
function xmlEscape(text){
    
        _yuitest_coverfunc("test", "xmlEscape", 406);
_yuitest_coverline("test", 408);
return text.replace(/[<>"'&]/g, function(value){
            _yuitest_coverfunc("test", "(anonymous 3)", 408);
_yuitest_coverline("test", 409);
switch(value){
                case "<":   _yuitest_coverline("test", 410);
return "&lt;";
                case ">":   _yuitest_coverline("test", 411);
return "&gt;";
                case "\"":  _yuitest_coverline("test", 412);
return "&quot;";
                case "'":   _yuitest_coverline("test", 413);
return "&apos;";
                case "&":   _yuitest_coverline("test", 414);
return "&amp;";
            }
        });
...
return {
        ...
        JSON: function(results) {
            _yuitest_coverfunc("test", "JSON", 430);
_yuitest_coverline("test", 431);
return YUITest.Util.JSON.stringify(results);
        ...
        XML: function(results) {
            ...
            _yuitest_coverfunc("test", "XML", 441);
_yuitest_coverline("test", 443);
function serializeToXML(results){
                _yuitest_coverfunc("test", "serializeToXML", 443);
_yuitest_coverline("test", 444);
var xml = "<" + results.type + " name=\"" + xmlEscape(results.name) + "\"";
                
                _yuitest_coverline("test", 446);
if (typeof(results.duration)=="number"){
                    _yuitest_coverline("test", 447);
xml += " duration=\"" + results.duration + "\"";
                }
                
                _yuitest_coverline("test", 450);
if (results.type == "test"){
                    _yuitest_coverline("test", 451);
xml += " result=\"" + results.result + "\" message=\"" + xmlEscape(results.message) + "\">";
                } else {
                    _yuitest_coverline("test", 453);
xml += " passed=\"" + results.passed + "\" failed=\"" + results.failed + "\" ignored=\"" + results.ignored + "\" total=\"" + results.total + "\">";
                    _yuitest_coverline("test", 454);
for (var prop in results){
                        _yuitest_coverline("test", 455);
if (results.hasOwnProperty(prop)){
                            _yuitest_coverline("test", 456);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                _yuitest_coverline("test", 457);
xml += serializeToXML(results[prop]);
                ...
                _yuitest_coverline("test", 463);
xml += "</" + results.type + ">";
                
                _yuitest_coverline("test", 465);
return xml;    
            }

            _yuitest_coverline("test", 468);
return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + serializeToXML(results);
        ...
        JUnitXML: function(results) {
            ...
            _yuitest_coverfunc("test", "JUnitXML", 480);
_yuitest_coverline("test", 482);
function serializeToJUnitXML(results){
                _yuitest_coverfunc("test", "serializeToJUnitXML", 482);
_yuitest_coverline("test", 483);
var xml = "";
                    
                _yuitest_coverline("test", 485);
switch (results.type){
                    //equivalent to testcase in JUnit
                    case "test":
                        _yuitest_coverline("test", 488);
if (results.result != "ignore"){
                            _yuitest_coverline("test", 489);
xml = "<testcase name=\"" + xmlEscape(results.name) + "\" time=\"" + (results.duration/1000) + "\">";
                            _yuitest_coverline("test", 490);
if (results.result == "fail"){
                                _yuitest_coverline("test", 491);
xml += "<failure message=\"" + xmlEscape(results.message) + "\"><![CDATA[" + results.message + "]]></failure>";
                            }
                            _yuitest_coverline("test", 493);
xml+= "</testcase>";
                        }
                        _yuitest_coverline("test", 495);
break;
                        
                    //equivalent to testsuite in JUnit
                    case "testcase":
                    
                        _yuitest_coverline("test", 500);
xml = "<testsuite name=\"" + xmlEscape(results.name) + "\" tests=\"" + results.total + "\" failures=\"" + results.failed + "\" time=\"" + (results.duration/1000) + "\">";
                        
                        _yuitest_coverline("test", 502);
for (var prop in results){
                            _yuitest_coverline("test", 503);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 504);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 505);
xml += serializeToJUnitXML(results[prop]);
                                }
                            }
                        }            
                        
                        _yuitest_coverline("test", 510);
xml += "</testsuite>";
                        _yuitest_coverline("test", 511);
break;
                    
                    //no JUnit equivalent, don't output anything
                    case "testsuite":
                        _yuitest_coverline("test", 515);
for (var prop in results){
                            _yuitest_coverline("test", 516);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 517);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 518);
xml += serializeToJUnitXML(results[prop]);
                                }
                            }
                        }                                                     
                        _yuitest_coverline("test", 522);
break;
                        
                    //top-level, equivalent to testsuites in JUnit
                    case "report":
                    
                        _yuitest_coverline("test", 527);
xml = "<testsuites>";
                    
                        _yuitest_coverline("test", 529);
for (var prop in results){
                            _yuitest_coverline("test", 530);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 531);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 532);
xml += serializeToJUnitXML(results[prop]);
                                }
                            }
                        }            
                        
                        _yuitest_coverline("test", 537);
xml += "</testsuites>";            
                    
                    //no default
                }
                
                _yuitest_coverline("test", 542);
return xml;
         
            }

            _yuitest_coverline("test", 546);
return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + serializeToJUnitXML(results);
        ...
        TAP: function(results) {
            ...
            _yuitest_coverfunc("test", "TAP", 557);
_yuitest_coverline("test", 559);
var currentTestNum = 1;

            _yuitest_coverline("test", 561);
function serializeToTAP(results){
                _yuitest_coverfunc("test", "serializeToTAP", 561);
_yuitest_coverline("test", 562);
var text = "";
                    
                _yuitest_coverline("test", 564);
switch (results.type){

                    case "test":
                        _yuitest_coverline("test", 567);
if (results.result != "ignore"){

                            _yuitest_coverline("test", 569);
text = "ok " + (currentTestNum++) + " - " + results.name;
                            
                            _yuitest_coverline("test", 571);
if (results.result == "fail"){
                                _yuitest_coverline("test", 572);
text = "not " + text + " - " + results.message;
                            }
                            
                            _yuitest_coverline("test", 575);
text += "\n";
                        } else {
                            _yuitest_coverline("test", 577);
text = "#Ignored test " + results.name + "\n";
                        }
                        _yuitest_coverline("test", 579);
break;
                        
                    case "testcase":
                    
                        _yuitest_coverline("test", 583);
text = "#Begin testcase " + results.name + "(" + results.failed + " failed of " + results.total + ")\n";
                                        
                        _yuitest_coverline("test", 585);
for (var prop in results){
                            _yuitest_coverline("test", 586);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 587);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 588);
text += serializeToTAP(results[prop]);
                                }
                            }
                        }            
                        
                        _yuitest_coverline("test", 593);
text += "#End testcase " + results.name + "\n";
                        
                        
                        _yuitest_coverline("test", 596);
break;
                    
                    case "testsuite":

                        _yuitest_coverline("test", 600);
text = "#Begin testsuite " + results.name + "(" + results.failed + " failed of " + results.total + ")\n";                
                    
                        _yuitest_coverline("test", 602);
for (var prop in results){
                            _yuitest_coverline("test", 603);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 604);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 605);
text += serializeToTAP(results[prop]);
                                }
                            }
                        }                                                      

                        _yuitest_coverline("test", 610);
text += "#End testsuite " + results.name + "\n";
                        _yuitest_coverline("test", 611);
break;

                    case "report":
                    
                        _yuitest_coverline("test", 615);
for (var prop in results){
                            _yuitest_coverline("test", 616);
if (results.hasOwnProperty(prop)){
                                _yuitest_coverline("test", 617);
if (results[prop] && typeof results[prop] == "object" && !(results[prop] instanceof Array)){
                                    _yuitest_coverline("test", 618);
text += serializeToTAP(results[prop]);
                                }
                            }
                        }              
                ...
                _yuitest_coverline("test", 626);
return text;
         
            }

            _yuitest_coverline("test", 630);
...
YUITest.Reporter = function(url, format) {
         ...
         * @property url
         */
        _yuitest_coverfunc("test", "Reporter", 646);
_yuitest_coverline("test", 653);
this.url = url;
...
this._form = null;
         ...
         * @private
         */
        _yuitest_coverline("test", 684);
this._iframe = null;
    };
    
    _yuitest_coverline("test", 687);
YUITest.Reporter.prototype = {
        ...
        addField : function (name, value){
            _yuitest_coverfunc("test", "addField", 699);
_yuitest_coverline("test", 700);
this._fields[name] = value;    
        ...
        clearFields : function(){
            _yuitest_coverfunc("test", "clearFields", 708);
_yuitest_coverline("test", 709);
this._fields = new Object();
        },
         ...
         */
        destroy : function() {
            _yuitest_coverfunc("test", "destroy", 718);
_yuitest_coverline("test", 719);
if (this._form){
                _yuitest_coverline("test", 720);
this._form.parentNode.removeChild(this._form);
                _yuitest_coverline("test", 721);
this._form = null;
            }        
            _yuitest_coverline("test", 723);
if (this._iframe){
                _yuitest_coverline("test", 724);
this._iframe.parentNode.removeChild(this._iframe);
                _yuitest_coverline("test", 725);
this._iframe = null;
            }
            _yuitest_coverline("test", 727);
this._fields = null;
        },
        ...
        report : function(results){
            ...
            _yuitest_coverfunc("test", "report", 736);
_yuitest_coverline("test", 739);
if (!this._form){
                _yuitest_coverline("test", 740);
this._form = document.createElement("form");
                _yuitest_coverline("test", 741);
this._form.method = "post";
                _yuitest_coverline("test", 742);
this._form.style.visibility = "hidden";
                _yuitest_coverline("test", 743);
this._form.style.position = "absolute";
                _yuitest_coverline("test", 744);
this._form.style.top = 0;
                _yuitest_coverline("test", 745);
document.body.appendChild(this._form);
            
                //IE won't let you assign a name using the DOM, must do it the hacky way
                _yuitest_coverline("test", 748);
try {
                    _yuitest_coverline("test", 749);
this._iframe = document.createElement("<iframe name=\"yuiTestTarget\" />");
                } catch (ex){
                    _yuitest_coverline("test", 751);
this._iframe = document.createElement("iframe");
                    _yuitest_coverline("test", 752);
this._iframe.name = "yuiTestTarget";
                }
    
                _yuitest_coverline("test", 755);
this._iframe.src = "javascript:false";
                _yuitest_coverline("test", 756);
this._iframe.style.visibility = "hidden";
                _yuitest_coverline("test", 757);
this._iframe.style.position = "absolute";
                _yuitest_coverline("test", 758);
this._iframe.style.top = 0;
                _yuitest_coverline("test", 759);
document.body.appendChild(this._iframe);
    
                _yuitest_coverline("test", 761);
this._form.target = "yuiTestTarget";
            }
    
            //set the form's action
            _yuitest_coverline("test", 765);
this._form.action = this.url;
        
            //remove any existing fields
            _yuitest_coverline("test", 768);
while(this._form.hasChildNodes()){
                _yuitest_coverline("test", 769);
this._form.removeChild(this._form.lastChild);
            }
            
            //create default fields
            _yuitest_coverline("test", 773);
this._fields.results = this.format(results);
            _yuitest_coverline("test", 774);
this._fields.useragent = navigator.userAgent;
            _yuitest_coverline("test", 775);
this._fields.timestamp = (new Date()).toLocaleString();
    
            //add fields to the form
            _yuitest_coverline("test", 778);
for (var prop in this._fields){
                _yuitest_coverline("test", 779);
var value = this._fields[prop];
                _yuitest_coverline("test", 780);
if (this._fields.hasOwnProperty(prop) && (typeof value != "function")){
                    _yuitest_coverline("test", 781);
var input = document.createElement("input");
                    _yuitest_coverline("test", 782);
input.type = "hidden";
                    _yuitest_coverline("test", 783);
input.name = prop;
                    _yuitest_coverline("test", 784);
input.value = value;
                    _yuitest_coverline("test", 785);
this._form.appendChild(input);
                }
            }
    
            //remove default fields
            _yuitest_coverline("test", 790);
delete this._fields.results;
            _yuitest_coverline("test", 791);
delete this._fields.useragent;
            _yuitest_coverline("test", 792);
delete this._fields.timestamp;
            
            _yuitest_coverline("test", 794);
if (arguments[1] !== false){
                _yuitest_coverline("test", 795);
this._form.submit();
            }
...
YUITest.TestRunner = function(){
         ...
         */
        _yuitest_coverfunc("test", "TestRunner", 810);
_yuitest_coverline("test", 819);
function inGroups(testGroups, filter){
            _yuitest_coverfunc("test", "inGroups", 819);
_yuitest_coverline("test", 820);
if (!filter.length){
                _yuitest_coverline("test", 821);
return true;
            } else {                
                _yuitest_coverline("test", 823);
if (testGroups){
                    _yuitest_coverline("test", 824);
for (var i=0, len=testGroups.length; i < len; i++){
                        _yuitest_coverline("test", 825);
if (filter.indexOf("," + testGroups[i] + ",") > -1){
                            _yuitest_coverline("test", 826);
return true;
                        }
                    }
                }
                _yuitest_coverline("test", 830);
return false;
            }
        }
...
function TestNode(testObject){
             ...
             * @property testObject
             */
            _yuitest_coverfunc("test", "TestNode", 843);
_yuitest_coverline("test", 850);
...
this.next = null;
             ...
             * @property results
             */                
            _yuitest_coverline("test", 885);
this.results = new YUITest.Results();
            
            //initialize results
            _yuitest_coverline("test", 888);
if (testObject instanceof YUITest.TestSuite){
                _yuitest_coverline("test", 889);
this.results.type = "testsuite";
                _yuitest_coverline("test", 890);
this.results.name = testObject.name;
            } else {_yuitest_coverline("test", 891);
if (testObject instanceof YUITest.TestCase){
                _yuitest_coverline("test", 892);
this.results.type = "testcase";
                _yuitest_coverline("test", 893);
this.results.name = testObject.name;
            }}
           
        }
        
        _yuitest_coverline("test", 898);
TestNode.prototype = {
            ...
            appendChild : function (testObject){
                _yuitest_coverfunc("test", "appendChild", 907);
_yuitest_coverline("test", 908);
var node = new TestNode(testObject);
                _yuitest_coverline("test", 909);
if (this.firstChild === null){
                    _yuitest_coverline("test", 910);
this.firstChild = this.lastChild = node;
                } else {
                    _yuitest_coverline("test", 912);
this.lastChild.next = node;
                    _yuitest_coverline("test", 913);
this.lastChild = node;
                }
                _yuitest_coverline("test", 915);
node.parent = this;
                _yuitest_coverline("test", 916);
return node;
            }       
        };
 ...
 * @class Runner
         * @static
         */
        _yuitest_coverline("test", 928);
function TestRunner(){
        
            //inherit from EventTarget
            _yuitest_coverfunc("test", "TestRunner", 928);
_yuitest_coverline("test", 931);
...
this._context = null;
             ...
             * @static
             */
            _yuitest_coverline("test", 1016);
this._groups = "";

        }
        
        _yuitest_coverline("test", 1020);
TestRunner.prototype = YUITest.Util.mix(new YUITest.EventTarget(), {
           ...
           _addTestCaseToTestTree : function (parentNode, testCase){
                ...
                //add the test suite
                _yuitest_coverfunc("test", "_addTestCaseToTestTree", 1124);
_yuitest_coverline("test", 1127);
var node = parentNode.appendChild(testCase),
                    prop,
                    testName;
                
                //iterate over the items in the test case
                _yuitest_coverline("test", 1132);
for (prop in testCase){
                    _yuitest_coverline("test", 1133);
if ((prop.indexOf("test") === 0 || prop.indexOf(" ") > -1) && typeof testCase[prop] == "function"){
                        _yuitest_coverline("test", 1134);
node.appendChild(prop);
            ...
            _addTestSuiteToTestTree : function (parentNode, testSuite) {
                ...
                //add the test suite
                _yuitest_coverfunc("test", "_addTestSuiteToTestTree", 1149);
_yuitest_coverline("test", 1152);
var node = parentNode.appendChild(testSuite);
                
                //iterate over the items in the master suite
                _yuitest_coverline("test", 1155);
for (var i=0; i < testSuite.items.length; i++){
                    _yuitest_coverline("test", 1156);
if (testSuite.items[i] instanceof YUITest.TestSuite) {
                        _yuitest_coverline("test", 1157);
this._addTestSuiteToTestTree(node, testSuite.items[i]);
                    } else {_yuitest_coverline("test", 1158);
if (testSuite.items[i] instanceof YUITest.TestCase) {
                        _yuitest_coverline("test", 1159);
this._addTestCaseToTestTree(node, testSuite.items[i]);
            ...
            _buildTestTree : function () {
                ...
                _yuitest_coverfunc("test", "_buildTestTree", 1173);
_yuitest_coverline("test", 1175);
this._root = new TestNode(this.masterSuite);
                //this._cur = this._root;
                
                //iterate over the items in the master suite
                _yuitest_coverline("test", 1179);
for (var i=0; i < this.masterSuite.items.length; i++){
                    _yuitest_coverline("test", 1180);
if (this.masterSuite.items[i] instanceof YUITest.TestSuite) {
                        _yuitest_coverline("test", 1181);
this._addTestSuiteToTestTree(this._root, this.masterSuite.items[i]);
                    } else {_yuitest_coverline("test", 1182);
if (this.masterSuite.items[i] instanceof YUITest.TestCase) {
                        _yuitest_coverline("test", 1183);
this._addTestCaseToTestTree(this._root, this.masterSuite.items[i]);
            ...
            _handleTestObjectComplete : function (node) {
                _yuitest_coverfunc("test", "_handleTestObjectComplete", 1201);
_yuitest_coverline("test", 1202);
var parentNode;
                
                _yuitest_coverline("test", 1204);
if (node && (typeof node.testObject == "object")) {
                    _yuitest_coverline("test", 1205);
parentNode = node.parent;
                
                    _yuitest_coverline("test", 1207);
if (parentNode){
                        _yuitest_coverline("test", 1208);
parentNode.results.include(node.results); 
                        _yuitest_coverline("test", 1209);
parentNode.results[node.testObject.name] = node.results;
                    }
                
                    _yuitest_coverline("test", 1212);
if (node.testObject instanceof YUITest.TestSuite){
                        _yuitest_coverline("test", 1213);
this._execNonTestMethod(node, "tearDown", false);
                        _yuitest_coverline("test", 1214);
node.results.duration = (new Date()) - node._start;
                        _yuitest_coverline("test", 1215);
this.fire({ type: this.TEST_SUITE_COMPLETE_EVENT, testSuite: node.testObject, results: node.results});
                    } else {_yuitest_coverline("test", 1216);
if (node.testObject instanceof YUITest.TestCase){
                        _yuitest_coverline("test", 1217);
this._execNonTestMethod(node, "destroy", false);
                        _yuitest_coverline("test", 1218);
node.results.duration = (new Date()) - node._start;
                        _yuitest_coverline("test", 1219);
this.fire({ type: this.TEST_CASE_COMPLETE_EVENT, testCase: node.testObject, results: node.results});
            ...
            _next : function () {
                ...
                _yuitest_coverfunc("test", "_next", 1235);
_yuitest_coverline("test", 1237);
if (this._cur === null){
                    _yuitest_coverline("test", 1238);
this._cur = this._root;
                } else {_yuitest_coverline("test", 1239);
if (this._cur.firstChild) {
                    _yuitest_coverline("test", 1240);
this._cur = this._cur.firstChild;
                } else {_yuitest_coverline("test", 1241);
if (this._cur.next) {
                    _yuitest_coverline("test", 1242);
this._cur = this._cur.next;            
                } else {
                    _yuitest_coverline("test", 1244);
while (this._cur && !this._cur.next && this._cur !== this._root){
                        _yuitest_coverline("test", 1245);
this._handleTestObjectComplete(this._cur);
                        _yuitest_coverline("test", 1246);
this._cur = this._cur.parent;
                    }
                    
                    _yuitest_coverline("test", 1249);
this._handleTestObjectComplete(this._cur);               
                        
                    _yuitest_coverline("test", 1251);
if (this._cur == this._root){
                        _yuitest_coverline("test", 1252);
this._cur.results.type = "report";
                        _yuitest_coverline("test", 1253);
this._cur.results.timestamp = (new Date()).toLocaleString();
                        _yuitest_coverline("test", 1254);
this._cur.results.duration = (new Date()) - this._cur._start;   
                        _yuitest_coverline("test", 1255);
this._lastResults = this._cur.results;
                        _yuitest_coverline("test", 1256);
this._running = false;                         
                        _yuitest_coverline("test", 1257);
this.fire({ type: this.COMPLETE_EVENT, results: this._lastResults});
                        _yuitest_coverline("test", 1258);
this._cur = null;
                    } else {_yuitest_coverline("test", 1259);
if (this._cur) {
                        _yuitest_coverline("test", 1260);
this._cur = this._cur.next;                
                    }}
                }}}
            
                _yuitest_coverline("test", 1264);
return this._cur;
            },
            ...
            _execNonTestMethod: function(node, methodName, allowAsync){
                _yuitest_coverfunc("test", "_execNonTestMethod", 1278);
_yuitest_coverline("test", 1279);
var testObject = node.testObject,
                    event = { type: this.ERROR_EVENT };
                _yuitest_coverline("test", 1281);
try {
                    _yuitest_coverline("test", 1282);
if (allowAsync && testObject["async:" + methodName]){
                        _yuitest_coverline("test", 1283);
testObject["async:" + methodName](this._context);
                        _yuitest_coverline("test", 1284);
return true;
                    } else {
                        _yuitest_coverline("test", 1286);
testObject[methodName](this._context);
                    }
                } catch (ex){
                    _yuitest_coverline("test", 1289);
node.results.errors++;
                    _yuitest_coverline("test", 1290);
event.error = ex;
                    _yuitest_coverline("test", 1291);
event.methodName = methodName;
                    _yuitest_coverline("test", 1292);
if (testObject instanceof YUITest.TestCase){
                        _yuitest_coverline("test", 1293);
event.testCase = testObject;
                    } else {
                        _yuitest_coverline("test", 1295);
event.testSuite = testSuite;
                    }
                    
                    _yuitest_coverline("test", 1298);
this.fire(event);
                }  

                _yuitest_coverline("test", 1301);
return false;
            },
            ...
            _run : function () {
                ...
                _yuitest_coverfunc("test", "_run", 1312);
_yuitest_coverline("test", 1315);
var shouldWait = false;
                
                //get the next test node
                _yuitest_coverline("test", 1318);
var node = this._next();
                
                _yuitest_coverline("test", 1320);
if (node !== null) {
                
                    //set flag to say the testrunner is running
                    _yuitest_coverline("test", 1323);
this._running = true;
                    
                    //eliminate last results
                    _yuitest_coverline("test", 1326);
this._lastResult = null;                  
                
                    _yuitest_coverline("test", 1328);
var testObject = node.testObject;
                    
                    //figure out what to do
                    _yuitest_coverline("test", 1331);
if (typeof testObject == "object" && testObject !== null){
                        _yuitest_coverline("test", 1332);
if (testObject instanceof YUITest.TestSuite){
                            _yuitest_coverline("test", 1333);
this.fire({ type: this.TEST_SUITE_BEGIN_EVENT, testSuite: testObject });
                            _yuitest_coverline("test", 1334);
node._start = new Date();
                            _yuitest_coverline("test", 1335);
this._execNonTestMethod(node, "setUp" ,false);
                        } else {_yuitest_coverline("test", 1336);
if (testObject instanceof YUITest.TestCase){
                            _yuitest_coverline("test", 1337);
this.fire({ type: this.TEST_CASE_BEGIN_EVENT, testCase: testObject });
                            _yuitest_coverline("test", 1338);
node._start = new Date();
                            ...
                            _yuitest_coverline("test", 1352);
if(this._execNonTestMethod(node, "init", true)){
                                _yuitest_coverline("test", 1353);
return;
                            }
                        }}
                        
                        //some environments don't support setTimeout
                        _yuitest_coverline("test", 1358);
if (typeof setTimeout != "undefined"){                    
                            _yuitest_coverline("test", 1359);
setTimeout(function(){
                                _yuitest_coverfunc("test", "(anonymous 4)", 1359);
_yuitest_coverline("test", 1360);
YUITest.TestRunner._run();
                            }, 0);
                        } else {
                            _yuitest_coverline("test", 1363);
this._run();
                        }
                    } else {
                        _yuitest_coverline("test", 1366);
this._runTest(node);
                    }
            ...
            _resumeTest : function (segment) {
                ...
                //get relevant information
                _yuitest_coverfunc("test", "_resumeTest", 1372);
_yuitest_coverline("test", 1375);
var node = this._cur;                
                
                //we know there's no more waiting now
                _yuitest_coverline("test", 1378);
this._waiting = false;
                
                //if there's no node, it probably means a wait() was called after resume()
                _yuitest_coverline("test", 1381);
if (!node){
                    ...
                    _yuitest_coverline("test", 1385);
return;
                }
                
                _yuitest_coverline("test", 1388);
var testName = node.testObject;
                _yuitest_coverline("test", 1389);
var testCase = node.parent.testObject;
            
                //cancel other waits if available
                _yuitest_coverline("test", 1392);
if (testCase.__yui_wait){
                    _yuitest_coverline("test", 1393);
clearTimeout(testCase.__yui_wait);
                    _yuitest_coverline("test", 1394);
delete testCase.__yui_wait;
                }

                //get the "should" test cases
                _yuitest_coverline("test", 1398);
var shouldFail = testName.indexOf("fail:") === 0 ||
                                    (testCase._should.fail || {})[testName];
                _yuitest_coverline("test", 1400);
var shouldError = (testCase._should.error || {})[testName];
                
                //variable to hold whether or not the test failed
                _yuitest_coverline("test", 1403);
var failed = false;
                _yuitest_coverline("test", 1404);
var error = null;
                    
                //try the test
                _yuitest_coverline("test", 1407);
try {
                
                    //run the test
                    _yuitest_coverline("test", 1410);
segment.call(testCase, this._context);                    
                
                    //if the test hasn't already failed and doesn't have any asserts...
                    _yuitest_coverline("test", 1413);
if(YUITest.Assert._getCount() == 0 && !this._ignoreEmpty){
                        _yuitest_coverline("test", 1414);
throw new YUITest.AssertionError("Test has no asserts.");
                    }                                                        
                    //if it should fail, and it got here, then it's a fail because it didn't
                     else {_yuitest_coverline("test", 1417);
if (shouldFail){
                        _yuitest_coverline("test", 1418);
error = new YUITest.ShouldFail();
                        _yuitest_coverline("test", 1419);
failed = true;
                    } else {_yuitest_coverline("test", 1420);
if (shouldError){
                        _yuitest_coverline("test", 1421);
error = new YUITest.ShouldError();
                        _yuitest_coverline("test", 1422);
failed = true;
                    }}}
                    ...
                    _yuitest_coverline("test", 1428);
if (testCase.__yui_wait){
                        _yuitest_coverline("test", 1429);
clearTimeout(testCase.__yui_wait);
                        _yuitest_coverline("test", 1430);
delete testCase.__yui_wait;
                    }                    
                
                    //figure out what type of error it was
                    _yuitest_coverline("test", 1434);
if (thrown instanceof YUITest.AssertionError) {
                        _yuitest_coverline("test", 1435);
if (!shouldFail){
                            _yuitest_coverline("test", 1436);
error = thrown;
                            _yuitest_coverline("test", 1437);
failed = true;
                        }
                    } else {_yuitest_coverline("test", 1439);
if (thrown instanceof YUITest.Wait){
                    
                        _yuitest_coverline("test", 1441);
if (typeof thrown.segment == "function"){
                            _yuitest_coverline("test", 1442);
if (typeof thrown.delay == "number"){
                            
                                //some environments don't support setTimeout
                                _yuitest_coverline("test", 1445);
if (typeof setTimeout != "undefined"){
                                    _yuitest_coverline("test", 1446);
testCase.__yui_wait = setTimeout(function(){
                                        _yuitest_coverfunc("test", "(anonymous 5)", 1446);
_yuitest_coverline("test", 1447);
YUITest.TestRunner._resumeTest(thrown.segment);
                                    }, thrown.delay);
                                    _yuitest_coverline("test", 1449);
this._waiting = true;
                                } else {
                                    _yuitest_coverline("test", 1451);
throw new Error("Asynchronous tests not supported in this environment.");
                                }
                            }
                        }
                        
                        _yuitest_coverline("test", 1456);
return;
                    
                    } else {
                        //first check to see if it should error
                        _yuitest_coverline("test", 1460);
if (!shouldError) {                        
                            _yuitest_coverline("test", 1461);
error = new YUITest.UnexpectedError(thrown);
                            _yuitest_coverline("test", 1462);
failed = true;
                        } else {
                            //check to see what type of data we have
                            _yuitest_coverline("test", 1465);
if (typeof shouldError == "string"){
                                
                                //if it's a string, check the error message
                                _yuitest_coverline("test", 1468);
if (thrown.message != shouldError){
                                    _yuitest_coverline("test", 1469);
error = new YUITest.UnexpectedError(thrown);
                                    _yuitest_coverline("test", 1470);
failed = true;                                    
                                }
                            } else {_yuitest_coverline("test", 1472);
if (typeof shouldError == "function"){
                            
                                //if it's a function, see if the error is an instance of it
                                _yuitest_coverline("test", 1475);
if (!(thrown instanceof shouldError)){
                                    _yuitest_coverline("test", 1476);
error = new YUITest.UnexpectedError(thrown);
                                    _yuitest_coverline("test", 1477);
failed = true;
                                }
                            
                            } else {_yuitest_coverline("test", 1480);
if (typeof shouldError == "object" && shouldError !== null){
                            
                                //if it's an object, check the instance and message
                                _yuitest_coverline("test", 1483);
if (!(thrown instanceof shouldError.constructor) || 
                                        thrown.message != shouldError.message){
                                    _yuitest_coverline("test", 1485);
error = new YUITest.UnexpectedError(thrown);
                                    _yuitest_coverline("test", 1486);
failed = true;                                    
                                }
...
node.parent.results[testName] = { 
                ...
                _yuitest_coverline("test", 1521);
if (failed){
                    _yuitest_coverline("test", 1522);
node.parent.results.failed++;
                } else {
                    _yuitest_coverline("test", 1524);
node.parent.results.passed++;
                }
                _yuitest_coverline("test", 1526);
node.parent.results.total++;
    
                //set timeout not supported in all environments
                _yuitest_coverline("test", 1529);
if (typeof setTimeout != "undefined"){
                    _yuitest_coverline("test", 1530);
setTimeout(function(){
                        _yuitest_coverfunc("test", "(anonymous 6)", 1530);
_yuitest_coverline("test", 1531);
YUITest.TestRunner._run();
                    }, 0);
                } else {
                    _yuitest_coverline("test", 1534);
this._run();
                }
            ...
            _handleError: function(error){
                ...
                _yuitest_coverfunc("test", "_handleError", 1552);
_yuitest_coverline("test", 1554);
if (this._waiting){
                    _yuitest_coverline("test", 1555);
this._resumeTest(function(){
                        _yuitest_coverfunc("test", "(anonymous 7)", 1555);
_yuitest_coverline("test", 1556);
throw error;
                    });
                } else {
                    _yuitest_coverline("test", 1559);
throw error;
                }           
            ...
            _runTest : function (node) {
                ...
                //get relevant information
                _yuitest_coverfunc("test", "_runTest", 1572);
_yuitest_coverline("test", 1575);
var testName = node.testObject,
                ...
                _yuitest_coverline("test", 1585);
if (shouldIgnore){
                
                    //update results
                    _yuitest_coverline("test", 1588);
node.parent.results[testName] = { 
                        result: "ignore",
                    ...
                    _yuitest_coverline("test", 1595);
node.parent.results.ignored++;
                    _yuitest_coverline("test", 1596);
node.parent.results.total++;
                
                    _yuitest_coverline("test", 1598);
this.fire({ type: this.TEST_IGNORE_EVENT,  testCase: testCase, testName: testName });
                    
                    //some environments don't support setTimeout
                    _yuitest_coverline("test", 1601);
if (typeof setTimeout != "undefined"){                    
                        _yuitest_coverline("test", 1602);
setTimeout(function(){
                            _yuitest_coverfunc("test", "(anonymous 8)", 1602);
_yuitest_coverline("test", 1603);
YUITest.TestRunner._run();
                        }, 0);              
                    } else {
                        _yuitest_coverline("test", 1606);
this._run();
                    }
                    ...
                    //mark the start time
                    _yuitest_coverline("test", 1612);
node._start = new Date();
                
                    //run the setup
                    _yuitest_coverline("test", 1615);
this._execNonTestMethod(node.parent, "setUp", false);
                    
                    //now call the body of the test
                    _yuitest_coverline("test", 1618);
this._resumeTest(test);                
             ...
             */
            getName: function(){
                _yuitest_coverfunc("test", "getName", 1632);
_yuitest_coverline("test", 1633);
return this.masterSuite.name;
            ...
            setName: function(name){
                _yuitest_coverfunc("test", "setName", 1643);
_yuitest_coverline("test", 1644);
this.masterSuite.name = name;
             ...
             */
            add : function (testObject) {
                _yuitest_coverfunc("test", "add", 1658);
_yuitest_coverline("test", 1659);
this.masterSuite.add(testObject);
                _yuitest_coverline("test", 1660);
return this;
            },
             ...
             */
            clear : function () {
                _yuitest_coverfunc("test", "clear", 1669);
_yuitest_coverline("test", 1670);
this.masterSuite = new YUITest.TestSuite(YUITest.guid('testSuite_'));
            },
             ...
             */
            isWaiting: function() {
                _yuitest_coverfunc("test", "isWaiting", 1679);
_yuitest_coverline("test", 1680);
return this._waiting;
             ...
             */
            isRunning: function(){
                _yuitest_coverfunc("test", "isRunning", 1689);
_yuitest_coverline("test", 1690);
return this._running;
            ...
            getResults: function(format){
                _yuitest_coverfunc("test", "getResults", 1702);
_yuitest_coverline("test", 1703);
if (!this._running && this._lastResults){
                    _yuitest_coverline("test", 1704);
if (typeof format == "function"){
                        _yuitest_coverline("test", 1705);
return format(this._lastResults);                    
                    } else {
                        _yuitest_coverline("test", 1707);
return this._lastResults;
                    }
                } else {
                    _yuitest_coverline("test", 1710);
return null;
                }
            },            
            ...
            getCoverage: function(format){
                _yuitest_coverfunc("test", "getCoverage", 1724);
_yuitest_coverline("test", 1725);
if (!this._running && typeof _yuitest_coverage == "object"){
                    _yuitest_coverline("test", 1726);
if (typeof format == "function"){
                        _yuitest_coverline("test", 1727);
return format(_yuitest_coverage);                    
                    } else {
                        _yuitest_coverline("test", 1729);
return _yuitest_coverage;
                    }
                } else {
                    _yuitest_coverline("test", 1732);
return null;
                }            
            },
             ...
             */
            callback: function(){
                _yuitest_coverfunc("test", "callback", 1747);
_yuitest_coverline("test", 1748);
var names   = arguments,
                    data    = this._context,
                    that    = this;
                    
                _yuitest_coverline("test", 1752);
return function(){
                    _yuitest_coverfunc("test", "(anonymous 9)", 1752);
_yuitest_coverline("test", 1753);
for (var i=0; i < arguments.length; i++){
                        _yuitest_coverline("test", 1754);
data[names[i]] = arguments[i];
                    }
                    _yuitest_coverline("test", 1756);
that._run();
                };
            },
             ...
             */
            resume : function (segment) {
                _yuitest_coverfunc("test", "resume", 1768);
_yuitest_coverline("test", 1769);
if (this._waiting){
                    _yuitest_coverline("test", 1770);
this._resumeTest(segment || function(){});
                } else {
                    _yuitest_coverline("test", 1772);
throw new Error("resume() called without wait().");
            ...
            run : function (options) {
                ...
                _yuitest_coverfunc("test", "run", 1786);
_yuitest_coverline("test", 1788);
options = options || {};
                
                //pointer to runner to avoid scope issues 
                _yuitest_coverline("test", 1791);
var runner  = YUITest.TestRunner,
                    oldMode = options.oldMode;
                
                
                //if there's only one suite on the masterSuite, move it up
                _yuitest_coverline("test", 1796);
if (!oldMode && this.masterSuite.items.length == 1 && this.masterSuite.items[0] instanceof YUITest.TestSuite){
                    _yuitest_coverline("test", 1797);
this.masterSuite = this.masterSuite.items[0];
                }                
                
                //determine if there are any groups to filter on
                _yuitest_coverline("test", 1801);
runner._groups = (options.groups instanceof Array) ? "," + options.groups.join(",") + "," : "";
                
                //initialize the runner
                _yuitest_coverline("test", 1804);
runner._buildTestTree();
                _yuitest_coverline("test", 1805);
runner._context = {};
                _yuitest_coverline("test", 1806);
runner._root._start = new Date();
                
                //fire the begin event
                _yuitest_coverline("test", 1809);
runner.fire(runner.BEGIN_EVENT);
           
                //begin the testing
                _yuitest_coverline("test", 1812);
runner._run();
            }    
        });
        
        _yuitest_coverline("test", 1816);
...
YUITest.ArrayAssert = {
    ...
    _indexOf: function(haystack, needle){
        _yuitest_coverfunc("test", "_indexOf", 1844);
_yuitest_coverline("test", 1845);
if (haystack.indexOf){
            _yuitest_coverline("test", 1846);
return haystack.indexOf(needle);
        } else {
            _yuitest_coverline("test", 1848);
for (var i=0; i < haystack.length; i++){
                _yuitest_coverline("test", 1849);
if (haystack[i] === needle){
                    _yuitest_coverline("test", 1850);
return i;
                }
            }
            _yuitest_coverline("test", 1853);
return -1;
        }
    },
    ...
    _some: function(haystack, matcher){
        _yuitest_coverfunc("test", "_some", 1867);
_yuitest_coverline("test", 1868);
if (haystack.some){
            _yuitest_coverline("test", 1869);
return haystack.some(matcher);
        } else {
            _yuitest_coverline("test", 1871);
for (var i=0; i < haystack.length; i++){
                _yuitest_coverline("test", 1872);
if (matcher(haystack[i])){
                    _yuitest_coverline("test", 1873);
return true;
                }
            }
            _yuitest_coverline("test", 1876);
return false;
        }
    },    
    ...
    contains : function (needle, haystack, 
        ...
        _yuitest_coverfunc("test", "contains", 1889);
_yuitest_coverline("test", 1892);
YUITest.Assert._increment();               

        _yuitest_coverline("test", 1894);
if (this._indexOf(haystack, needle) == -1){
            _yuitest_coverline("test", 1895);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value " + needle + " (" + (typeof needle) + ") not found in array [" + haystack + "]."));
    ...
    containsItems : function (needles, haystack, 
                           message) {
        _yuitest_coverfunc("test", "containsItems", 1909);
_yuitest_coverline("test", 1911);
YUITest.Assert._increment();               

        //begin checking values
        _yuitest_coverline("test", 1914);
for (var i=0; i < needles.length; i++){
            _yuitest_coverline("test", 1915);
if (this._indexOf(haystack, needles[i]) == -1){
                _yuitest_coverline("test", 1916);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value " + needles[i] + " (" + (typeof needles[i]) + ") not found in array [" + haystack + "]."));
    ...
    containsMatch : function (matcher, haystack, 
        ...
        _yuitest_coverfunc("test", "containsMatch", 1930);
_yuitest_coverline("test", 1933);
YUITest.Assert._increment();               
        //check for valid matcher
        _yuitest_coverline("test", 1935);
if (typeof matcher != "function"){
            _yuitest_coverline("test", 1936);
throw new TypeError("ArrayAssert.containsMatch(): First argument must be a function.");
        }
        
        _yuitest_coverline("test", 1939);
if (!this._some(haystack, matcher)){
            _yuitest_coverline("test", 1940);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "No match found in array [" + haystack + "]."));
    ...
    doesNotContain : function (needle, haystack, 
        ...
        _yuitest_coverfunc("test", "doesNotContain", 1954);
_yuitest_coverline("test", 1957);
YUITest.Assert._increment();               

        _yuitest_coverline("test", 1959);
if (this._indexOf(haystack, needle) > -1){
            _yuitest_coverline("test", 1960);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value found in array [" + haystack + "]."));
    ...
    doesNotContainItems : function (needles, haystack, 
        ...
        _yuitest_coverfunc("test", "doesNotContainItems", 1974);
_yuitest_coverline("test", 1977);
YUITest.Assert._increment();               

        _yuitest_coverline("test", 1979);
for (var i=0; i < needles.length; i++){
            _yuitest_coverline("test", 1980);
if (this._indexOf(haystack, needles[i]) > -1){
                _yuitest_coverline("test", 1981);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value found in array [" + haystack + "]."));
            }
        }
    ...
    doesNotContainMatch : function (matcher, haystack, 
        ...
        _yuitest_coverfunc("test", "doesNotContainMatch", 1996);
_yuitest_coverline("test", 1999);
YUITest.Assert._increment();     
      
        //check for valid matcher
        _yuitest_coverline("test", 2002);
if (typeof matcher != "function"){
            _yuitest_coverline("test", 2003);
throw new TypeError("ArrayAssert.doesNotContainMatch(): First argument must be a function.");
        }
        
        _yuitest_coverline("test", 2006);
if (this._some(haystack, matcher)){
            _yuitest_coverline("test", 2007);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value found in array [" + haystack + "]."));
        }
    },
    ...
    indexOf : function (needle, haystack, index, message) {
        ...
        _yuitest_coverfunc("test", "indexOf", 2021);
_yuitest_coverline("test", 2023);
YUITest.Assert._increment();     

        //try to find the value in the array
        _yuitest_coverline("test", 2026);
for (var i=0; i < haystack.length; i++){
            _yuitest_coverline("test", 2027);
if (haystack[i] === needle){
                _yuitest_coverline("test", 2028);
if (index != i){
                    _yuitest_coverline("test", 2029);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value exists at index " + i + " but should be at index " + index + "."));                    
                }
                _yuitest_coverline("test", 2031);
return;
            }
        }
        
        //if it makes it here, it wasn't found at all
        _yuitest_coverline("test", 2036);
...
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Array should have a length of " + expected.length + " but has a length of " + actual.length + "."));
        ...
        //begin checking values
        _yuitest_coverline("test", 2066);
for (var i=0; i < expected.length; i++){
            _yuitest_coverline("test", 2067);
if (expected[i] != actual[i]){
                _yuitest_coverline("test", 2068);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values in position " + i + " are not equal."), expected[i], actual[i]);
    ...
    itemsAreEquivalent : function (expected, actual, 
        ...
        _yuitest_coverfunc("test", "itemsAreEquivalent", 2087);
_yuitest_coverline("test", 2090);
YUITest.Assert._increment();     

        //make sure the comparator is valid
        _yuitest_coverline("test", 2093);
if (typeof comparator != "function"){
            _yuitest_coverline("test", 2094);
throw new TypeError("ArrayAssert.itemsAreEquivalent(): Third argument must be a function.");
        }
        
        //first check array length
        _yuitest_coverline("test", 2098);
if (expected.length != actual.length){
            _yuitest_coverline("test", 2099);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Array should have a length of " + expected.length + " but has a length of " + actual.length));
        }
        
        //begin checking values
        _yuitest_coverline("test", 2103);
for (var i=0; i < expected.length; i++){
            _yuitest_coverline("test", 2104);
if (!comparator(expected[i], actual[i])){
                _yuitest_coverline("test", 2105);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values in position " + i + " are not equivalent."), expected[i], actual[i]);
    ...
    isEmpty : function (actual, message) {        
        _yuitest_coverfunc("test", "isEmpty", 2117);
_yuitest_coverline("test", 2118);
YUITest.Assert._increment();     
        _yuitest_coverline("test", 2119);
if (actual.length > 0){
            _yuitest_coverline("test", 2120);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Array should be empty."));
    ...
    isNotEmpty : function (actual, message) {        
        _yuitest_coverfunc("test", "isNotEmpty", 2131);
_yuitest_coverline("test", 2132);
YUITest.Assert._increment();     
        _yuitest_coverline("test", 2133);
if (actual.length === 0){
            _yuitest_coverline("test", 2134);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Array should not be empty."));
    ...
    itemsAreSame : function (expected, actual, 
        ...
        _yuitest_coverfunc("test", "itemsAreSame", 2149);
_yuitest_coverline("test", 2152);
YUITest.Assert._increment();     

        //first check array length
        _yuitest_coverline("test", 2155);
if (expected.length != actual.length){
            _yuitest_coverline("test", 2156);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Array should have a length of " + expected.length + " but has a length of " + actual.length));
        }
                    
        //begin checking values
        _yuitest_coverline("test", 2160);
for (var i=0; i < expected.length; i++){
            _yuitest_coverline("test", 2161);
if (expected[i] !== actual[i]){
                _yuitest_coverline("test", 2162);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values in position " + i + " are not the same."), expected[i], actual[i]);
    ...
    lastIndexOf : function (needle, haystack, index, message) {
        ...
        //try to find the value in the array
        _yuitest_coverfunc("test", "lastIndexOf", 2178);
_yuitest_coverline("test", 2181);
for (var i=haystack.length; i >= 0; i--){
            _yuitest_coverline("test", 2182);
if (haystack[i] === needle){
                _yuitest_coverline("test", 2183);
if (index != i){
                    _yuitest_coverline("test", 2184);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Value exists at index " + i + " but should be at index " + index + "."));                    
                }
                _yuitest_coverline("test", 2186);
return;
            }
        }
        
        //if it makes it here, it wasn't found at all
        _yuitest_coverline("test", 2191);
...
YUITest.Assert = {
    ...
    _formatMessage : function (customMessage, defaultMessage) {
        _yuitest_coverfunc("test", "_formatMessage", 2229);
_yuitest_coverline("test", 2230);
if (typeof customMessage == "string" && customMessage.length > 0){
            _yuitest_coverline("test", 2231);
return customMessage.replace("{message}", defaultMessage);
        } else {
            _yuitest_coverline("test", 2233);
return defaultMessage;
     ...
     */
    _getCount: function(){
        _yuitest_coverfunc("test", "_getCount", 2243);
_yuitest_coverline("test", 2244);
return this._asserts;
     ...
     */
    _increment: function(){
        _yuitest_coverfunc("test", "_increment", 2253);
_yuitest_coverline("test", 2254);
this._asserts++;
    },
     ...
     */
    _reset: function(){
        _yuitest_coverfunc("test", "_reset", 2263);
_yuitest_coverline("test", 2264);
this._asserts = 0;
    },
     ...
     */
    fail : function (message) {
        _yuitest_coverfunc("test", "fail", 2277);
_yuitest_coverline("test", 2278);
throw new YUITest.AssertionError(YUITest.Assert._formatMessage(message, "Test force-failed."));
    ...
    pass : function (message) {
        _yuitest_coverfunc("test", "pass", 2286);
_yuitest_coverline("test", 2287);
YUITest.Assert._increment();
    },       
    ...
    areEqual : function (expected, actual, message) {
        _yuitest_coverfunc("test", "areEqual", 2303);
_yuitest_coverline("test", 2304);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2305);
if (expected != actual) {
            _yuitest_coverline("test", 2306);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values should be equal."), expected, actual);
        }
    },
    ...
    areNotEqual : function (unexpected, actual, 
                         message) {
        _yuitest_coverfunc("test", "areNotEqual", 2319);
_yuitest_coverline("test", 2321);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2322);
if (unexpected == actual) {
            _yuitest_coverline("test", 2323);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Values should not be equal."), unexpected);
    ...
    areNotSame : function (unexpected, actual, message) {
        _yuitest_coverfunc("test", "areNotSame", 2336);
_yuitest_coverline("test", 2337);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2338);
if (unexpected === actual) {
            _yuitest_coverline("test", 2339);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Values should not be the same."), unexpected);
    ...
    areSame : function (expected, actual, message) {
        _yuitest_coverfunc("test", "areSame", 2352);
_yuitest_coverline("test", 2353);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2354);
if (expected !== actual) {
            _yuitest_coverline("test", 2355);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values should be the same."), expected, actual);
        }
    },    
    ...
    isFalse : function (actual, message) {
        _yuitest_coverfunc("test", "isFalse", 2371);
_yuitest_coverline("test", 2372);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2373);
if (false !== actual) {
            _yuitest_coverline("test", 2374);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be false."), false, actual);
    ...
    isTrue : function (actual, message) {
        _yuitest_coverfunc("test", "isTrue", 2386);
_yuitest_coverline("test", 2387);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2388);
if (true !== actual) {
            _yuitest_coverline("test", 2389);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be true."), true, actual);
    ...
    isNaN : function (actual, message){
        _yuitest_coverfunc("test", "isNaN", 2405);
_yuitest_coverline("test", 2406);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2407);
if (!isNaN(actual)){
            _yuitest_coverline("test", 2408);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be NaN."), NaN, actual);
    ...
    isNotNaN : function (actual, message){
        _yuitest_coverfunc("test", "isNotNaN", 2419);
_yuitest_coverline("test", 2420);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2421);
if (isNaN(actual)){
            _yuitest_coverline("test", 2422);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Values should not be NaN."), NaN);
    ...
    isNotNull : function (actual, message) {
        _yuitest_coverfunc("test", "isNotNull", 2434);
_yuitest_coverline("test", 2435);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2436);
if (actual === null) {
            _yuitest_coverline("test", 2437);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Values should not be null."), null);
    ...
    isNotUndefined : function (actual, message) {
        _yuitest_coverfunc("test", "isNotUndefined", 2449);
_yuitest_coverline("test", 2450);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2451);
if (typeof actual == "undefined") {
            _yuitest_coverline("test", 2452);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should not be undefined."), undefined);
    ...
    isNull : function (actual, message) {
        _yuitest_coverfunc("test", "isNull", 2464);
_yuitest_coverline("test", 2465);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2466);
if (actual !== null) {
            _yuitest_coverline("test", 2467);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be null."), null, actual);
    ...
    isUndefined : function (actual, message) {
        _yuitest_coverfunc("test", "isUndefined", 2479);
_yuitest_coverline("test", 2480);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2481);
if (typeof actual != "undefined") {
            _yuitest_coverline("test", 2482);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be undefined."), undefined, actual);
    ...
    isArray : function (actual, message) {
        _yuitest_coverfunc("test", "isArray", 2497);
_yuitest_coverline("test", 2498);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2499);
var shouldFail = false;
        _yuitest_coverline("test", 2500);
if (Array.isArray){
            _yuitest_coverline("test", 2501);
shouldFail = !Array.isArray(actual);
        } else {
            _yuitest_coverline("test", 2503);
shouldFail = Object.prototype.toString.call(actual) != "[object Array]";
        }
        _yuitest_coverline("test", 2505);
if (shouldFail){
            _yuitest_coverline("test", 2506);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be an array."), actual);
    ...
    isBoolean : function (actual, message) {
        _yuitest_coverfunc("test", "isBoolean", 2517);
_yuitest_coverline("test", 2518);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2519);
if (typeof actual != "boolean"){
            _yuitest_coverline("test", 2520);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be a Boolean."), actual);
    ...
    isFunction : function (actual, message) {
        _yuitest_coverfunc("test", "isFunction", 2531);
_yuitest_coverline("test", 2532);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2533);
if (!(actual instanceof Function)){
            _yuitest_coverline("test", 2534);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be a function."), actual);
    ...
    isInstanceOf : function (expected, actual, message) {
        _yuitest_coverfunc("test", "isInstanceOf", 2548);
_yuitest_coverline("test", 2549);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2550);
if (!(actual instanceof expected)){
            _yuitest_coverline("test", 2551);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value isn't an instance of expected type."), expected, actual);
    ...
    isNumber : function (actual, message) {
        _yuitest_coverfunc("test", "isNumber", 2562);
_yuitest_coverline("test", 2563);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2564);
if (typeof actual != "number"){
            _yuitest_coverline("test", 2565);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be a number."), actual);
    ...
    isObject : function (actual, message) {
        _yuitest_coverfunc("test", "isObject", 2576);
_yuitest_coverline("test", 2577);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2578);
if (!actual || (typeof actual != "object" && typeof actual != "function")){
            _yuitest_coverline("test", 2579);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be an object."), actual);
    ...
    isString : function (actual, message) {
        _yuitest_coverfunc("test", "isString", 2590);
_yuitest_coverline("test", 2591);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2592);
if (typeof actual != "string"){
            _yuitest_coverline("test", 2593);
throw new YUITest.UnexpectedValue(YUITest.Assert._formatMessage(message, "Value should be a string."), actual);
    ...
    isTypeOf : function (expectedType, actualValue, message){
        _yuitest_coverfunc("test", "isTypeOf", 2605);
_yuitest_coverline("test", 2606);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2607);
if (typeof actualValue != expectedType){
            _yuitest_coverline("test", 2608);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Value should be of type " + expectedType + "."), expectedType, typeof actualValue);
    ...
    throwsError: function(expectedError, method, message){
        _yuitest_coverfunc("test", "throwsError", 2631);
_yuitest_coverline("test", 2632);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2633);
var error = false;
    
        _yuitest_coverline("test", 2635);
try {
            _yuitest_coverline("test", 2636);
method();        
        } catch (thrown) {
            
            //check to see what type of data we have
            _yuitest_coverline("test", 2640);
if (typeof expectedError == "string"){
                
                //if it's a string, check the error message
                _yuitest_coverline("test", 2643);
if (thrown.message != expectedError){
                    _yuitest_coverline("test", 2644);
error = true;
                }
            } else {_yuitest_coverline("test", 2646);
if (typeof expectedError == "function"){
            
                //if it's a function, see if the error is an instance of it
                _yuitest_coverline("test", 2649);
if (!(thrown instanceof expectedError)){
                    _yuitest_coverline("test", 2650);
error = true;
                }
            
            } else {_yuitest_coverline("test", 2653);
if (typeof expectedError == "object" && expectedError !== null){
            
                //if it's an object, check the instance and message
                _yuitest_coverline("test", 2656);
if (!(thrown instanceof expectedError.constructor) || 
                        thrown.message != expectedError.message){
                    _yuitest_coverline("test", 2658);
error = true;
                }
            
            } else { //if it gets here, the argument could be wrong
                _yuitest_coverline("test", 2662);
error = true;
            }}}
            
            _yuitest_coverline("test", 2665);
if (error){
                _yuitest_coverline("test", 2666);
throw new YUITest.UnexpectedError(thrown);                    
            } else {
                _yuitest_coverline("test", 2668);
return;
            }
        }
        
        //if it reaches here, the error wasn't thrown, which is a bad thing
        _yuitest_coverline("test", 2673);
...
YUITest.AssertionError = function (message){
     ...
     * @property message
     */
    _yuitest_coverfunc("test", "AssertionError", 2688);
_yuitest_coverline("test", 2695);
this.message = message;
     ...
     * @property name
     */
    _yuitest_coverline("test", 2702);
this.name = "Assert Error";
};

_yuitest_coverline("test", 2705);
YUITest.AssertionError.prototype = {
     ...
     */
    getMessage : function () {
        _yuitest_coverfunc("test", "getMessage", 2716);
_yuitest_coverline("test", 2717);
return this.message;
    },
     ...
     */
    toString : function () {
        _yuitest_coverfunc("test", "toString", 2725);
_yuitest_coverline("test", 2726);
 ...
 * @constructor
 */ 
_yuitest_coverline("test", 2743);
YUITest.ComparisonFailure = function (message, expected, actual){

    //call superclass
    _yuitest_coverfunc("test", "ComparisonFailure", 2743);
_yuitest_coverline("test", 2746);
 ...
 * @return {String} A string describing the error.
 */
_yuitest_coverline("test", 2783);
YUITest.ComparisonFailure.prototype.getMessage = function(){
    _yuitest_coverfunc("test", "getMessage", 2783);
_yuitest_coverline("test", 2784);
return this.message + "\nExpected: " + this.expected + " (" + (typeof this.expected) + ")"  +
...
YUITest.CoverageFormat = {
    ...
    JSON: function(coverage){
        _yuitest_coverfunc("test", "JSON", 2804);
_yuitest_coverline("test", 2805);
return YUITest.Util.JSON.stringify(coverage);
    ...
    XdebugJSON: function(coverage){
        ...
        _yuitest_coverfunc("test", "XdebugJSON", 2818);
_yuitest_coverline("test", 2820);
var report = {};
        _yuitest_coverline("test", 2821);
for (var prop in coverage){
            _yuitest_coverline("test", 2822);
if (coverage.hasOwnProperty(prop)){
                _yuitest_coverline("test", 2823);
report[prop] = coverage[prop].lines;
            }
        }

        _yuitest_coverline("test", 2827);
return YUITest.Util.JSON.stringify(coverage);
    }
...
YUITest.DateAssert = {
    ...
    datesAreEqual : function (expected, actual, message){
        _yuitest_coverfunc("test", "datesAreEqual", 2851);
_yuitest_coverline("test", 2852);
YUITest.Assert._increment();        
        _yuitest_coverline("test", 2853);
if (expected instanceof Date && actual instanceof Date){
            _yuitest_coverline("test", 2854);
var msg = "";
            
            //check years first
            _yuitest_coverline("test", 2857);
if (expected.getFullYear() != actual.getFullYear()){
                _yuitest_coverline("test", 2858);
msg = "Years should be equal.";
            }
            
            //now check months
            _yuitest_coverline("test", 2862);
if (expected.getMonth() != actual.getMonth()){
                _yuitest_coverline("test", 2863);
msg = "Months should be equal.";
            }                
            
            //last, check the day of the month
            _yuitest_coverline("test", 2867);
if (expected.getDate() != actual.getDate()){
                _yuitest_coverline("test", 2868);
msg = "Days of month should be equal.";
            }                
            
            _yuitest_coverline("test", 2871);
if (msg.length){
                _yuitest_coverline("test", 2872);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, msg), expected, actual);
            }
        } else {
            _yuitest_coverline("test", 2875);
throw new TypeError("YUITest.DateAssert.datesAreEqual(): Expected and actual values must be Date objects.");
    ...
    timesAreEqual : function (expected, actual, message){
        _yuitest_coverfunc("test", "timesAreEqual", 2887);
_yuitest_coverline("test", 2888);
YUITest.Assert._increment();
        _yuitest_coverline("test", 2889);
if (expected instanceof Date && actual instanceof Date){
            _yuitest_coverline("test", 2890);
var msg = "";
            
            //check hours first
            _yuitest_coverline("test", 2893);
if (expected.getHours() != actual.getHours()){
                _yuitest_coverline("test", 2894);
msg = "Hours should be equal.";
            }
            
            //now check minutes
            _yuitest_coverline("test", 2898);
if (expected.getMinutes() != actual.getMinutes()){
                _yuitest_coverline("test", 2899);
msg = "Minutes should be equal.";
            }                
            
            //last, check the seconds
            _yuitest_coverline("test", 2903);
if (expected.getSeconds() != actual.getSeconds()){
                _yuitest_coverline("test", 2904);
msg = "Seconds should be equal.";
            }                
            
            _yuitest_coverline("test", 2907);
if (msg.length){
                _yuitest_coverline("test", 2908);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, msg), expected, actual);
            }
        } else {
            _yuitest_coverline("test", 2911);
 ...
 */
_yuitest_coverline("test", 2924);
YUITest.Mock = function(template){

    //use blank object is nothing is passed in
    _yuitest_coverfunc("test", "Mock", 2924);
_yuitest_coverline("test", 2927);
template = template || {};
    
    _yuitest_coverline("test", 2929);
var mock,
        name;
    
    //try to create mock that keeps prototype chain intact
    //fails in the case of ActiveX objects
    _yuitest_coverline("test", 2934);
try {
        _yuitest_coverline("test", 2935);
function f(){}
        _yuitest_coverline("test", 2936);
f.prototype = template;
        _yuitest_coverline("test", 2937);
mock = new f();
    } catch (ex) {
        _yuitest_coverline("test", 2939);
mock = {};
    }

    //create stubs for all methods
    _yuitest_coverline("test", 2943);
for (name in template){
        _yuitest_coverline("test", 2944);
if (template.hasOwnProperty(name)){
            _yuitest_coverline("test", 2945);
if (typeof template[name] == "function"){
                _yuitest_coverline("test", 2946);
mock[name] = function(name){
                    _yuitest_coverfunc("test", "]", 2946);
_yuitest_coverline("test", 2947);
return function(){
                        _yuitest_coverfunc("test", "(anonymous 11)", 2947);
_yuitest_coverline("test", 2948);
 ...
 * @static
 */ 
_yuitest_coverline("test", 2979);
YUITest.Mock.expect = function(mock /*:Object*/, expectation /*:Object*/){

    //make sure there's a place to store the expectations
    _yuitest_coverfunc("test", "expect", 2979);
_yuitest_coverline("test", 2982);
if (!mock.__expectations) {
        _yuitest_coverline("test", 2983);
mock.__expectations = {};
    }

    //method expectation
    _yuitest_coverline("test", 2987);
if (expectation.method){
        _yuitest_coverline("test", 2988);
var name = expectation.method,
        ...
        //save expectations
        _yuitest_coverline("test", 2998);
mock.__expectations[name] = expectation;
        _yuitest_coverline("test", 2999);
expectation.callCount = callCount;
        _yuitest_coverline("test", 3000);
expectation.actualCallCount = 0;
            
        //process arguments
        _yuitest_coverline("test", 3003);
for (i=0; i < args.length; i++){
             _yuitest_coverline("test", 3004);
if (!(args[i] instanceof YUITest.Mock.Value)){
                _yuitest_coverline("test", 3005);
args[i] = YUITest.Mock.Value(YUITest.Assert.areSame, [args[i]], "Argument " + i + " of " + name + "() is incorrect.");
            }       
        }
    
        //if the method is expected to be called
        _yuitest_coverline("test", 3010);
if (callCount > 0){
            _yuitest_coverline("test", 3011);
mock[name] = function(){   
                _yuitest_coverfunc("test", "]", 3011);
_yuitest_coverline("test", 3012);
try {
                    _yuitest_coverline("test", 3013);
expectation.actualCallCount++;
                    _yuitest_coverline("test", 3014);
YUITest.Assert.areEqual(args.length, arguments.length, "Method " + name + "() passed incorrect number of arguments.");
                    _yuitest_coverline("test", 3015);
for (var i=0, len=args.length; i < len; i++){
                        _yuitest_coverline("test", 3016);
args[i].verify(arguments[i]);
                    }                

                    _yuitest_coverline("test", 3019);
runResult = run.apply(this, arguments);
                    
                    _yuitest_coverline("test", 3021);
if (error){
                        _yuitest_coverline("test", 3022);
throw error;
                    }
                } catch (ex){
                    //route through TestRunner for proper handling
                    _yuitest_coverline("test", 3026);
YUITest.TestRunner._handleError(ex);
                }

                // Any value provided for 'returns' overrides any value returned
                // by our 'run' function. 
                _yuitest_coverline("test", 3031);
return expectation.hasOwnProperty('returns') ? result : runResult;
            };
        } else {
        
            //method should fail if called when not expected
            _yuitest_coverline("test", 3036);
mock[name] = function(){
                _yuitest_coverfunc("test", "]", 3036);
_yuitest_coverline("test", 3037);
try {
                    _yuitest_coverline("test", 3038);
YUITest.Assert.fail("Method " + name + "() should not have been called.");
                } catch (ex){
                    //route through TestRunner for proper handling
                    _yuitest_coverline("test", 3041);
YUITest.TestRunner._handleError(ex);
                }                    
            };
        }
    } else {_yuitest_coverline("test", 3045);
if (expectation.property){
        //save expectations
        _yuitest_coverline("test", 3047);
 ...
 * @static
 */ 
_yuitest_coverline("test", 3059);
YUITest.Mock.verify = function(mock){    
    _yuitest_coverfunc("test", "verify", 3059);
_yuitest_coverline("test", 3060);
try {
    
        _yuitest_coverline("test", 3062);
for (var name in mock.__expectations){
            _yuitest_coverline("test", 3063);
if (mock.__expectations.hasOwnProperty(name)){
                _yuitest_coverline("test", 3064);
var expectation = mock.__expectations[name];
                _yuitest_coverline("test", 3065);
if (expectation.method) {
                    _yuitest_coverline("test", 3066);
YUITest.Assert.areEqual(expectation.callCount, expectation.actualCallCount, "Method " + expectation.method + "() wasn't called the expected number of times.");
                } else {_yuitest_coverline("test", 3067);
if (expectation.property){
                    _yuitest_coverline("test", 3068);
 ...
 * @constructor
 */
_yuitest_coverline("test", 3089);
YUITest.Mock.Value = function(method, originalArgs, message){
    _yuitest_coverfunc("test", "Value", 3089);
_yuitest_coverline("test", 3090);
if (this instanceof YUITest.Mock.Value){
        _yuitest_coverline("test", 3091);
this.verify = function(value){
            _yuitest_coverfunc("test", "verify", 3091);
_yuitest_coverline("test", 3092);
var args = [].concat(originalArgs || []);
            _yuitest_coverline("test", 3093);
args.push(value);
            _yuitest_coverline("test", 3094);
args.push(message);
            _yuitest_coverline("test", 3095);
method.apply(null, args);
        };
    } else {
        _yuitest_coverline("test", 3098);
...
YUITest.ObjectAssert = {
     ...
     */
    areEqual: function(expected, actual, message) {
        _yuitest_coverfunc("test", "areEqual", 3170);
_yuitest_coverline("test", 3171);
YUITest.Assert._increment();         
        
        _yuitest_coverline("test", 3173);
var expectedKeys = YUITest.Object.keys(expected),
            actualKeys = YUITest.Object.keys(actual);
        
        //first check keys array length
        _yuitest_coverline("test", 3177);
if (expectedKeys.length != actualKeys.length){
            _yuitest_coverline("test", 3178);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Object should have " + expectedKeys.length + " keys but has " + actualKeys.length));
        ...
        //then check values
        _yuitest_coverline("test", 3182);
for (var name in expected){
            _yuitest_coverline("test", 3183);
if (expected.hasOwnProperty(name)){
                _yuitest_coverline("test", 3184);
if (expected[name] != actual[name]){
                    _yuitest_coverline("test", 3185);
throw new YUITest.ComparisonFailure(YUITest.Assert._formatMessage(message, "Values should be equal for property " + name), expected[name], actual[name]);
    ...
    hasKey: function (propertyName, object, message) {
        _yuitest_coverfunc("test", "hasKey", 3200);
_yuitest_coverline("test", 3201);
YUITest.ObjectAssert.ownsOrInheritsKey(propertyName, object, message);   
    ...
    hasKeys: function (properties, object, message) {
        _yuitest_coverfunc("test", "hasKeys", 3213);
_yuitest_coverline("test", 3214);
YUITest.ObjectAssert.ownsOrInheritsKeys(properties, object, message);
    ...
    inheritsKey: function (propertyName, object, message) {
        _yuitest_coverfunc("test", "inheritsKey", 3225);
_yuitest_coverline("test", 3226);
YUITest.Assert._increment();               
        _yuitest_coverline("test", 3227);
if (!(propertyName in object && !object.hasOwnProperty(propertyName))){
            _yuitest_coverline("test", 3228);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Property '" + propertyName + "' not found on object instance."));
    ...
    inheritsKeys: function (properties, object, message) {
        _yuitest_coverfunc("test", "inheritsKeys", 3240);
_yuitest_coverline("test", 3241);
YUITest.Assert._increment();        
        _yuitest_coverline("test", 3242);
for (var i=0; i < properties.length; i++){
            _yuitest_coverline("test", 3243);
if (!(propertyName in object && !object.hasOwnProperty(properties[i]))){
                _yuitest_coverline("test", 3244);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Property '" + properties[i] + "' not found on object instance."));
    ...
    ownsKey: function (propertyName, object, message) {
        _yuitest_coverfunc("test", "ownsKey", 3257);
_yuitest_coverline("test", 3258);
YUITest.Assert._increment();               
        _yuitest_coverline("test", 3259);
if (!object.hasOwnProperty(propertyName)){
            _yuitest_coverline("test", 3260);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Property '" + propertyName + "' not found on object instance."));
        }     
    },
    ...
    ownsKeys: function (properties, object, message) {
        _yuitest_coverfunc("test", "ownsKeys", 3272);
_yuitest_coverline("test", 3273);
YUITest.Assert._increment();        
        _yuitest_coverline("test", 3274);
for (var i=0; i < properties.length; i++){
            _yuitest_coverline("test", 3275);
if (!object.hasOwnProperty(properties[i])){
                _yuitest_coverline("test", 3276);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Property '" + properties[i] + "' not found on object instance."));
            }      
        }
    },
    ...
    ownsNoKeys : function (object, message) {
        _yuitest_coverfunc("test", "ownsNoKeys", 3288);
_yuitest_coverline("test", 3289);
YUITest.Assert._increment();  
        _yuitest_coverline("test", 3290);
var count = 0,
            name;
        _yuitest_coverline("test", 3292);
for (name in object){
            _yuitest_coverline("test", 3293);
if (object.hasOwnProperty(name)){
                _yuitest_coverline("test", 3294);
count++;
            }
        }
        
        _yuitest_coverline("test", 3298);
if (count !== 0){
            _yuitest_coverline("test", 3299);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Object owns " + count + " properties but should own none."));        
    ...
    ownsOrInheritsKey: function (propertyName, object, message) {
        _yuitest_coverfunc("test", "ownsOrInheritsKey", 3312);
_yuitest_coverline("test", 3313);
YUITest.Assert._increment();               
        _yuitest_coverline("test", 3314);
if (!(propertyName in object)){
            _yuitest_coverline("test", 3315);
YUITest.Assert.fail(YUITest.Assert._formatMessage(message, "Property '" + propertyName + "' not found on object."));
    ...
    ownsOrInheritsKeys: function (properties, object, message) {
        _yuitest_coverfunc("test", "ownsOrInheritsKeys", 3327);
_yuitest_coverline("test", 3328);
YUITest.Assert._increment();  
        _yuitest_coverline("test", 3329);
for (var i=0; i < properties.length; i++){
            _yuitest_coverline("test", 3330);
if (!(properties[i] in object)){
                _yuitest_coverline("test", 3331);
...
YUITest.Results = function(name){
     ...
     * @property name
     */
    _yuitest_coverfunc("test", "Results", 3346);
_yuitest_coverline("test", 3353);
this.name = name;
 ...
 * @return {void}
 */
_yuitest_coverline("test", 3404);
YUITest.Results.prototype.include = function(results){
    _yuitest_coverfunc("test", "include", 3404);
_yuitest_coverline("test", 3405);
this.passed += results.passed;
    _yuitest_coverline("test", 3406);
this.failed += results.failed;
    _yuitest_coverline("test", 3407);
this.ignored += results.ignored;
    _yuitest_coverline("test", 3408);
this.total += results.total;
    _yuitest_coverline("test", 3409);
 ...
 * @constructor
 */ 
_yuitest_coverline("test", 3421);
YUITest.ShouldError = function (message){

    //call superclass
    _yuitest_coverfunc("test", "ShouldError", 3421);
_yuitest_coverline("test", 3424);
YUITest.AssertionError.call(this, message || "This test should have thrown an error but didn't.");
     ...
     * @property name
     */
    _yuitest_coverline("test", 3431);
this.name = "ShouldError";
    
};

//inherit from YUITest.AssertionError
_yuitest_coverline("test", 3436);
YUITest.ShouldError.prototype = new YUITest.AssertionError();

//restore constructor
_yuitest_coverline("test", 3439);
 ...
 * @constructor
 */ 
_yuitest_coverline("test", 3451);
YUITest.ShouldFail = function (message){

    //call superclass
    _yuitest_coverfunc("test", "ShouldFail", 3451);
_yuitest_coverline("test", 3454);
YUITest.AssertionError.call(this, message || "This test should fail but didn't.");
     ...
     * @property name
     */
    _yuitest_coverline("test", 3461);
this.name = "ShouldFail";
    
};

//inherit from YUITest.AssertionError
_yuitest_coverline("test", 3466);
YUITest.ShouldFail.prototype = new YUITest.AssertionError();

//restore constructor
_yuitest_coverline("test", 3469);
 ...
 * @constructor
 */  
_yuitest_coverline("test", 3483);
YUITest.UnexpectedError = function (cause){

    //call superclass
    _yuitest_coverfunc("test", "UnexpectedError", 3483);
_yuitest_coverline("test", 3486);
...
this.name = "UnexpectedError";
     ...
     * @property stack
     */
    _yuitest_coverline("test", 3507);
this.stack = cause.stack;
    
};

//inherit from YUITest.AssertionError
_yuitest_coverline("test", 3512);
YUITest.UnexpectedError.prototype = new YUITest.AssertionError();

//restore constructor
_yuitest_coverline("test", 3515);
 ...
 * @constructor
 */ 
_yuitest_coverline("test", 3530);
YUITest.UnexpectedValue = function (message, unexpected){

    //call superclass
    _yuitest_coverfunc("test", "UnexpectedValue", 3530);
_yuitest_coverline("test", 3533);
YUITest.AssertionError.call(this, message);
...
this.unexpected = unexpected;
     ...
     * @property name
     */
    _yuitest_coverline("test", 3547);
this.name = "UnexpectedValue";
    
};

//inherit from YUITest.AssertionError
_yuitest_coverline("test", 3552);
YUITest.UnexpectedValue.prototype = new YUITest.AssertionError();

//restore constructor
_yuitest_coverline("test", 3555);
 ...
 * @return {String} A string describing the error.
 */
_yuitest_coverline("test", 3563);
YUITest.UnexpectedValue.prototype.getMessage = function(){
    _yuitest_coverfunc("test", "getMessage", 3563);
_yuitest_coverline("test", 3564);
...
YUITest.Wait = function (segment, delay) {
     ...
     * @property segment
     */
    _yuitest_coverfunc("test", "Wait", 3578);
_yuitest_coverline("test", 3585);
this.segment = (typeof segment == "function" ? segment : null);
     ...
     * @property delay
     */
    _yuitest_coverline("test", 3592);
this.delay = (typeof delay == "number" ? delay : 0);        
};


//Setting up our aliases..
_yuitest_coverline("test", 3597);
Y.Test = YUITest;
_yuitest_coverline("test", 3598);
Y.Object.each(YUITest, function(item, name) {
    _yuitest_coverfunc("test", "(anonymous 14)", 3598);
_yuitest_coverline("test", 3599);
var name = name.replace('Test', '');
    _yuitest_coverline("test", 3600);
Y.Test[name] = item;
});

} //End of else in top wrapper

_yuitest_coverline("test", 3605);
Y.Assert = YUITest.Assert;
_yuitest_coverline("test", 3606);
Y.Assert.Error = Y.Test.AssertionError;
_yuitest_coverline("test", 3607);
Y.Assert.ComparisonFailure = Y.Test.ComparisonFailure;
_yuitest_coverline("test", 3608);
Y.Assert.UnexpectedValue = Y.Test.UnexpectedValue;
_yuitest_coverline("test", 3609);
Y.Mock = Y.Test.Mock;
_yuitest_coverline("test", 3610);
Y.ObjectAssert = Y.Test.ObjectAssert;
_yuitest_coverline("test", 3611);
Y.ArrayAssert = Y.Test.ArrayAssert;
_yuitest_coverline("test", 3612);
Y.DateAssert = Y.Test.DateAssert;
_yuitest_coverline("test", 3613);
Y.Test.ResultsFormat = Y.Test.TestFormat;

_yuitest_coverline("test", 3615);
var itemsAreEqual = Y.Test.ArrayAssert.itemsAreEqual;

_yuitest_coverline("test", 3617);
Y.Test.ArrayAssert.itemsAreEqual = function(expected, actual, message) {
    _yuitest_coverfunc("test", "itemsAreEqual", 3617);
_yuitest_coverline("test", 3618);
 ...
 * @static
 */
_yuitest_coverline("test", 3631);
Y.assert = function(condition, message){
    _yuitest_coverfunc("test", "assert", 3631);
_yuitest_coverline("test", 3632);
Y.Assert._increment();
    _yuitest_coverline("test", 3633);
if (!condition){
        _yuitest_coverline("test", 3634);
 ...
 * @static
 */
_yuitest_coverline("test", 3645);
Y.fail = Y.Assert.fail; 

_yuitest_coverline("test", 3647);
Y.Test.Runner.once = Y.Test.Runner.subscribe;

_yuitest_coverline("test", 3649);
Y.Test.Runner.disableLogging = function() {
    _yuitest_coverfunc("test", "disableLogging", 3649);
_yuitest_coverline("test", 3650);
Y.Test.Runner._log = false;
};

_yuitest_coverline("test", 3653);
Y.Test.Runner.enableLogging = function() {
    _yuitest_coverfunc("test", "enableLogging", 3653);
_yuitest_coverline("test", 3654);
Y.Test.Runner._log = true;
};

_yuitest_coverline("test", 3657);
Y.Test.Runner._ignoreEmpty = true;
_yuitest_coverline("test", 3658);
Y.Test.Runner._log = true;

_yuitest_coverline("test", 3660);
Y.Test.Runner.on = Y.Test.Runner.attach;

//Only allow one instance of YUITest
_yuitest_coverline("test", 3663);
if (!YUI.YUITest) {

    _yuitest_coverline("test", 3665);
if (Y.config.win) {
        _yuitest_coverline("test", 3666);
Y.config.win.YUITest = YUITest;
    }

    _yuitest_coverline("test", 3669);
YUI.YUITest = Y.Test;

    
    //Only setup the listeners once.
    _yuitest_coverline("test", 3673);
var logEvent = function(event) {
        
        //data variables
        _yuitest_coverfunc("test", "logEvent", 3673);
_yuitest_coverline("test", 3676);
var message = "";
        _yuitest_coverline("test", 3677);
var messageType = "";
        
        _yuitest_coverline("test", 3679);
switch(event.type){
            case this.BEGIN_EVENT:
                _yuitest_coverline("test", 3681);
message = "Testing began at " + (new Date()).toString() + ".";
                _yuitest_coverline("test", 3682);
messageType = "info";
                _yuitest_coverline("test", 3683);
break;
                
            case this.COMPLETE_EVENT:
                _yuitest_coverline("test", 3686);
message = Y.substitute("Testing completed at " +
                    (new Date()).toString() + ".\n" +
                    "Passed:{passed} Failed:{failed} " +
                    "Total:{total} ({ignored} ignored)",
                    event.results);
                _yuitest_coverline("test", 3691);
messageType = "info";
                _yuitest_coverline("test", 3692);
break;
                
            case this.TEST_FAIL_EVENT:
                _yuitest_coverline("test", 3695);
message = event.testName + ": failed.\n" + event.error.getMessage();
                _yuitest_coverline("test", 3696);
messageType = "fail";
                _yuitest_coverline("test", 3697);
break;
                
            case this.TEST_IGNORE_EVENT:
                _yuitest_coverline("test", 3700);
message = event.testName + ": ignored.";
                _yuitest_coverline("test", 3701);
messageType = "ignore";
                _yuitest_coverline("test", 3702);
break;
                
            case this.TEST_PASS_EVENT:
                _yuitest_coverline("test", 3705);
message = event.testName + ": passed.";
                _yuitest_coverline("test", 3706);
messageType = "pass";
                _yuitest_coverline("test", 3707);
break;
                
            case this.TEST_SUITE_BEGIN_EVENT:
                _yuitest_coverline("test", 3710);
message = "Test suite \"" + event.testSuite.name + "\" started.";
                _yuitest_coverline("test", 3711);
messageType = "info";
                _yuitest_coverline("test", 3712);
break;
                
            case this.TEST_SUITE_COMPLETE_EVENT:
                _yuitest_coverline("test", 3715);
message = Y.substitute("Test suite \"" +
                    event.testSuite.name + "\" completed" + ".\n" +
                    "Passed:{passed} Failed:{failed} " +
                    "Total:{total} ({ignored} ignored)",
                    event.results);
                _yuitest_coverline("test", 3720);
messageType = "info";
                _yuitest_coverline("test", 3721);
break;
                
            case this.TEST_CASE_BEGIN_EVENT:
                _yuitest_coverline("test", 3724);
message = "Test case \"" + event.testCase.name + "\" started.";
                _yuitest_coverline("test", 3725);
messageType = "info";
                _yuitest_coverline("test", 3726);
break;
                
            case this.TEST_CASE_COMPLETE_EVENT:
                _yuitest_coverline("test", 3729);
message = Y.substitute("Test case \"" +
                    event.testCase.name + "\" completed.\n" +
                    "Passed:{passed} Failed:{failed} " +
                    "Total:{total} ({ignored} ignored)",
                    event.results);
                _yuitest_coverline("test", 3734);
messageType = "info";
                _yuitest_coverline("test", 3735);
break;
            default:
                _yuitest_coverline("test", 3737);
message = "Unexpected event " + event.type;
                _yuitest_coverline("test", 3738);
message = "info";
        }
        
        _yuitest_coverline("test", 3741);
if (Y.Test.Runner._log) {
            _yuitest_coverline("test", 3742);
Y.log(message, messageType, "TestRunner");
        }
    }

    _yuitest_coverline("test", 3746);
var i, name;

    _yuitest_coverline("test", 3748);
for (i in Y.Test.Runner) {
        _yuitest_coverline("test", 3749);
name = Y.Test.Runner[i];
        _yuitest_coverline("test", 3750);
if (i.indexOf('_EVENT') > -1) {
            _yuitest_coverline("test", 3751);
Y.Test.Runner.subscribe(name, logEvent);
        }
    }_yuitest_coverline("test", 3753);
;
