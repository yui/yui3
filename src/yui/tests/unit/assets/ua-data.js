YUI.add("ua-data", function(Y) {

// THIS FILE SHOULD BE HAND VERIFIED BEFORE TESTING

Y.UAData = {
	"Chrome": [
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/7.0.540.0 Safari/534.10",
			"data": {
				"webkit": 534.1,
				"chrome": 7.054,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.19 (KHTML, like Gecko) Chrome/1.0.154.36 Safari/525.19",
			"data": {
				"webkit": 525.19,
				"chrome": 1.0154,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US)  AppleWebKit/534.13 (KHTML, like Gecko) Chrome/9.0.597.0 Safari/534.13",
			"data": {
				"webkit": 534.13,
				"chrome": 9.0597,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/532.2 (KHTML, like Gecko) Chrome/4.0.223.3 Safari/532.2",
			"data": {
				"webkit": 532.2,
				"chrome": 4.0223,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.0) AppleWebKit/535.1 (KHTML, like Gecko) Chrome/14.0.792.0 Safari/535.1",
			"data": {
				"webkit": 535.1,
				"chrome": 14.0792,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Windows NT 6; en-US) AppleWebKit/534.12 (KHTML, like Gecko) Chrome/9.0.587.0 Safari/534.12",
			"data": {
				"webkit": 534.12,
				"chrome": 9.0587,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/530.5 (KHTML, like Gecko) Chrome/2.0.173.1 Safari/530.5",
			"data": {
				"webkit": 530.5,
				"chrome": 2.0173,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.2; en-US) AppleWebKit/534.4 (KHTML, like Gecko) Chrome/6.0.481.0 Safari/534.4",
			"data": {
				"webkit": 534.4,
				"chrome": 6.0481,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/534.16 (KHTML, like Gecko) Chrome/10.0.648.11 Safari/534.16",
			"data": {
				"webkit": 534.16,
				"chrome": 10.0648,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.2 (KHTML, like Gecko) Chrome/15.0.872.0 Safari/535.2",
			"data": {
				"webkit": 535.2,
				"chrome": 15.0872,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US) AppleWebKit/540.0 (KHTML,like Gecko) Chrome/9.1.0.0 Safari/540.0",
			"data": {
				"webkit": 540,
				"chrome": 9.1,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/532.0 (KHTML, like Gecko) Chrome/4.0.201.1 Safari/532.0",
			"data": {
				"webkit": 532,
				"chrome": 4.0201,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.19 (KHTML, like Gecko) Chrome/1.0.154.53 Safari/525.19",
			"data": {
				"webkit": 525.19,
				"chrome": 1.0154,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.2; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/8.0.558.0 Safari/534.10",
			"data": {
				"webkit": 534.1,
				"chrome": 8.0558,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.7 (KHTML, like Gecko) Chrome/16.0.912.36 Safari/535.7",
			"data": {
				"webkit": 535.7,
				"chrome": 16.0912,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; Intel Mac OS X; en-US) AppleWebKit/533.4 (KHTML, like Gecko) Chrome/5.0.375.86 Safari/533.4",
			"data": {
				"webkit": 533.4,
				"chrome": 5.0375,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US) AppleWebKit/534.20 (KHTML, like Gecko) Chrome/11.0.672.2 Safari/534.20",
			"data": {
				"webkit": 534.2,
				"chrome": 11.0672,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/534.14 (KHTML, like Gecko) Chrome/9.0.600.0 Safari/534.14",
			"data": {
				"webkit": 534.14,
				"chrome": 9.06,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.2; en-US) AppleWebKit/532.0 (KHTML, like Gecko) Chrome/3.0.195.27 Safari/532.0",
			"data": {
				"webkit": 532,
				"chrome": 3.0195,
				"os": "windows"
			}
		}
	],
	"Chromium": [
		{
			"ua": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/535.1 (KHTML, like Gecko) Ubuntu/10.04 Chromium/14.0.813.0 Chrome/14.0.813.0 Safari/535.1",
			"data": {
				"webkit": 535.1,
				"chrome": 14.0813,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/535.7 (KHTML, like Gecko) Ubuntu/11.10 Chromium/16.0.912.21 Chrome/16.0.912.21 Safari/535.7",
			"data": {
				"webkit": 535.7,
				"chrome": 16.0912,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/534.30 (KHTML, like Gecko) Ubuntu/10.10 Chromium/12.0.742.112 Chrome/12.0.742.112 Safari/534.30",
			"data": {
				"webkit": 534.3,
				"chrome": 12.0742,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Ubuntu/10.10 Chromium/8.0.552.237 Chrome/8.0.552.237 Safari/534.10",
			"data": {
				"webkit": 534.1,
				"chrome": 8.0552,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/535.2 (KHTML, like Gecko) Ubuntu/11.04 Chromium/15.0.871.0 Chrome/15.0.871.0 Safari/535.2",
			"data": {
				"webkit": 535.2,
				"chrome": 15.0871,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Ubuntu/10.10 Chromium/8.0.552.237 Chrome/8.0.552.237 Safari/534.10",
			"data": {
				"webkit": 534.1,
				"chrome": 8.0552,
				"os": "linux"
			}
		}
	],
	"Firefox": [
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:2.0b9pre) Gecko/20101228 Firefox/4.0b9pre",
			"data": {
				"gecko": 2,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.1b3) Gecko/20090305 Firefox/3.1b3 GTB5",
			"data": {
				"gecko": 1.91,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.8.1.12) Gecko/20080214 Firefox/2.0.0.12",
			"data": {
				"gecko": 1.81,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:2.0b8pre) Gecko/20101114 Firefox/4.0b8pre",
			"data": {
				"gecko": 2,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux amd64; rv:5.0) Gecko/20100101 Firefox/5.0 (Debian)",
			"data": {
				"gecko": 5,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.0; es-ES; rv:1.8.0.3) Gecko/20060426 Firefox/1.5.0.3",
			"data": {
				"gecko": 1.8,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; ko; rv:1.9.1b2) Gecko/20081201 Firefox/3.1b2",
			"data": {
				"gecko": 1.91,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; Windows NT 6.1; rv:2.0b2) Gecko/20100720 Firefox/4.0b2",
			"data": {
				"gecko": 2,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; Linux x86_64; rv:2.0b4) Gecko/20100818 Firefox/4.0b4",
			"data": {
				"gecko": 2,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; cs; rv:1.9.0.8) Gecko/2009032609 Firefox/3.0.8",
			"data": {
				"gecko": 1.9,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:6.0a2) Gecko/20110613 Firefox/6.0a2",
			"data": {
				"gecko": 6,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.2) Gecko/20100308 Ubuntu/10.04 (lucid) Firefox/3.6 GTB7.1",
			"data": {
				"gecko": 1.92,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; WinNT4.0; en-US; rv:1.7.9) Gecko/20050711 Firefox/1.0.5",
			"data": {
				"gecko": 1.79,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; SunOS sun4u; en-US; rv:1.9b5) Gecko/2008032620 Firefox/3.0b5",
			"data": {
				"gecko": 1.9
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:2.0b7) Gecko/20101111 Firefox/4.0b7",
			"data": {
				"gecko": 2,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; Linux x86_64; rv:2.0b9pre) Gecko/20110111 Firefox/4.0b9pre",
			"data": {
				"gecko": 2,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:2.2a1pre) Gecko/20110324 Firefox/4.2a1pre",
			"data": {
				"gecko": 2.2,
				"os": "windows"
			}
		}
	],
	"Firefox (BonEcho)": [
		{
			"ua": "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.1.9) Gecko/20071113 BonEcho/2.0.0.9",
			"data": {
				"gecko": 1.81,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.1) Gecko/20061026 BonEcho/2.0",
			"data": {
				"gecko": 1.81,
				"os": "linux"
			}
		}
	],
	"Firefox (GranParadiso)": [
		{
			"ua": "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.0.8) Gecko/2009033017 GranParadiso/3.0.8",
			"data": {
				"gecko": 1.9,
				"os": "linux"
			}
		}
	],
	"Firefox (Lorentz)": [
		{
			"ua": "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; en-US; rv:1.9.2) Gecko/20100411 Lorentz/3.6.3 GTB7.0",
			"data": {
				"gecko": 1.92,
				"os": "macintosh"
			}
		}
	],
	"Firefox (Minefield)": [
		{
			"ua": "Mozilla/5.0 (X11; U; Linux i686; it; rv:1.9.3a1pre) Gecko/20091019 Minefield/3.7a1pre",
			"data": {
				"gecko": 1.93,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.0; en-US; rv:1.9.3a4pre) Gecko/20100402 Minefield/3.7a4pre",
			"data": {
				"gecko": 1.93,
				"os": "windows"
			}
		}
	],
	"Firefox (Namoroka)": [
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2a1) Gecko/20090806 Namoroka/3.6a1",
			"data": {
				"gecko": 1.92,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; cs; rv:1.9.2a2pre) Gecko/20090912 Namoroka/3.6a2pre (.NET CLR 3.5.30729)",
			"data": {
				"gecko": 1.92,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.9.2a2pre) Gecko/20090901 Ubuntu/9.10 (karmic) Namoroka/3.6a2pre",
			"data": {
				"gecko": 1.92,
				"os": "linux"
			}
		}
	],
	"Firefox (Shiretoko)": [
		{
			"ua": "Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.9.1b3pre) Gecko/20090109 Shiretoko/3.1b3pre",
			"data": {
				"gecko": 1.91,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US; rv:1.9.1b4pre) Gecko/20090311 Shiretoko/3.1b4pre",
			"data": {
				"gecko": 1.91,
				"os": "windows"
			}
		}
	],
	"IE": [
		{
			"ua": "Mozilla/4.0 (Windows; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 2.0.50727)",
			"data": {
				"ie": 6,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/2.0 (compatible; MSIE 4.0; Windows 98)",
			"data": {
				"ie": 4,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; SLCC1; .NET CLR 2.0.50727; .NET CLR 1.1.4322; InfoPath.2; .NET CLR 3.5.21022; .NET CLR 3.5.30729; MS-RTC LM 8; OfficeLiveConnector.1.4; OfficeLivePatch.1.3; .NET CLR 3.0.30729)",
			"data": {
				"ie": 8,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (Mozilla/4.0; MSIE 7.0; Windows NT 5.1; FDM; SV1; .NET CLR 3.0.04506.30)",
			"data": {
				"ie": 7,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0; SLCC2; Media Center PC 6.0; InfoPath.3; MS-RTC LM 8; Zune 4.7)",
			"data": {
				"ie": 9,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/6.0)",
			"data": {
				"ie": 10,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; WOW64; Trident/6.0)",
			"data": {
				"ie": 10,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.01; Windows NT)",
			"data": {
				"ie": 5.01,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0)",
			"data": {
				"ie": 9,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.0.3705; .NET CLR 1.1.4322; Media Center PC 4.0; .NET CLR 2.0.50727)",
			"data": {
				"ie": 6,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Win64; x64; Trident/6.0)",
			"data": {
				"ie": 10,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.0b1; Mac_PowerPC)",
			"data": {
				"ie": 5,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.0; Windows NT;)",
			"data": {
				"ie": 5,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; GTB5; SLCC1; .NET CLR 2.0.50727; Media Center PC 5.0; .NET CLR 3.0.04506; InfoPath.2; OfficeLiveConnector.1.3; OfficeLivePatch.0.0)",
			"data": {
				"ie": 8,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.23; Mac_PowerPC)",
			"data": {
				"ie": 5.23,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; GTB6; Ant.com Toolbar 1.6; MSIECrawler)",
			"data": {
				"ie": 6,
				"os": "windows"
			}
		}
	],
	"Opera": [
		{
			"ua": "Opera/9.80 (X11; Linux i686; U; ru) Presto/2.8.131 Version/11.11",
			"data": {
				"opera": 11.11,
				"os": "linux"
			}
		},
		{
			"ua": "Opera/9.80 (Windows NT 5.1; U; cs) Presto/2.2.15 Version/10.00",
			"data": {
				"opera": 10,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/7.03 (Windows NT 5.0; U) [en]",
			"data": {
				"opera": 7.03,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.80 (Windows NT 6.1; U; zh-tw) Presto/2.7.62 Version/11.01",
			"data": {
				"opera": 11.01,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; en) Opera 9.24",
			"data": {
				"opera": 9.24,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/6.0 (Windows 2000; U) [fr]",
			"data": {
				"opera": 6,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.80 (Windows NT 6.1; U; sv) Presto/2.7.62 Version/11.01",
			"data": {
				"opera": 11.01,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1) Opera 7.10 [en]",
			"data": {
				"opera": 7.1,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.51 (Macintosh; Intel Mac OS X; U; en)",
			"data": {
				"opera": 9.51,
				"os": "macintosh"
			}
		},
		{
			"ua": "Opera/5.11 (Windows 98; U) [en]",
			"data": {
				"opera": 5.11,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.80 (Windows NT 6.0; U; en) Presto/2.8.99 Version/11.10",
			"data": {
				"opera": 11.1,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.0; Windows NT 4.0) Opera 6.01 [en]",
			"data": {
				"opera": 6.01,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.80 (Windows NT 6.1; U; en-GB) Presto/2.7.62 Version/11.00",
			"data": {
				"opera": 11,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.02 (Windows XP; U; ru)",
			"data": {
				"opera": 9.02,
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.70 (Linux i686 ; U; en) Presto/2.2.1",
			"data": {
				"opera": 9.7,
				"os": "linux"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 5.0; Windows 98) Opera 5.12 [en]",
			"data": {
				"opera": 5.12,
				"os": "windows"
			}
		}
	],
	"Safari": [
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US) AppleWebKit/528.16 (KHTML, like Gecko) Version/4.0 Safari/528.16",
			"data": {
				"webkit": 528.16,
				"safari": 528.16,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; fi-fi) AppleWebKit/420+ (KHTML, like Gecko) Safari/419.3",
			"data": {
				"webkit": 420,
				"safari": 420,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; en-us) AppleWebKit/312.8 (KHTML, like Gecko) Safari/312.6",
			"data": {
				"webkit": 312.8,
				"safari": 312.8,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/533.20.25 (KHTML, like Gecko) Version/5.0.4 Safari/533.20.27",
			"data": {
				"webkit": 533.2025,
				"safari": 533.2025,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; PPC Mac OS X 10_5_6; it-it) AppleWebKit/528.16 (KHTML, like Gecko) Version/4.0 Safari/528.16",
			"data": {
				"webkit": 528.16,
				"safari": 528.16,
				"os": "macintosh"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; zh-HK) AppleWebKit/533.18.1 (KHTML, like Gecko) Version/5.0.2 Safari/533.18.5",
			"data": {
				"webkit": 533.181,
				"safari": 533.181,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; cs-CZ) AppleWebKit/523.15 (KHTML, like Gecko) Version/3.0 Safari/523.15",
			"data": {
				"webkit": 523.15,
				"safari": 523.15,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 6.1; sv-SE) AppleWebKit/533.19.4 (KHTML, like Gecko) Version/5.0.3 Safari/533.19.4",
			"data": {
				"webkit": 533.194,
				"safari": 533.194,
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; de-de) AppleWebKit/125.2 (KHTML, like Gecko) Safari/125.7",
			"data": {
				"webkit": 125.2,
				"safari": 125.2,
				"os": "macintosh"
			}
		}
	],
	"Android Webkit": [
		{
			"ua": "Mozilla/5.0 (Linux; U; Android 2.1-update1; en-au; HTC_Desire_A8183 V1.16.841.1 Build/ERE27) AppleWebKit/530.17 (KHTML, like Gecko) Version/4.0 Mobile Safari/530.17",
			"data": {
				"webkit": 530.17,
				"safari": 530.17,
				"mobile": "Android",
				"android": 2.1,
				"os": "android"
			}
		},
		{
			"ua": "Mozilla/5.0 (Linux; U; Android 1.5; de-de; HTC Magic Build/CRB17) AppleWebKit/528.5+ (KHTML, like Gecko) Version/3.1.2 Mobile Safari/525.20.1",
			"data": {
				"webkit": 528.5,
				"safari": 528.5,
				"mobile": "Android",
				"android": 1.5,
				"os": "android"
			}
		}
	],
	"IE Mobile": [
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 9.0; Windows Phone OS 7.5; Trident/5.0; IEMobile/9.0)",
			"data": {
				"ie": 9,
				"mobile": "windows",
				"os": "windows"
			}
		},
		{
			"ua": "HTC_P4550/TYTN_II Mozilla/4.0 (compatible; MSIE 6.0; Windows CE; IEMobile 7.11)",
			"data": {
				"ie": 6,
				"mobile": "windows",
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.1; XBLWP7; ZuneWP7)",
			"data": {
				"ie": 7,
				"mobile": "windows",
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/4.0 (compatible; MSIE 6.0; Windows CE; IEMobile 7.11) 320x240; VZW; Motorola-Q9c; Windows Mobile 6.1 Standard",
			"data": {
				"ie": 6,
				"mobile": "windows",
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0; XBLWP7; ZuneWP7)",
			"data": {
				"ie": 9,
				"mobile": "windows",
				"os": "windows"
			}
		},
		{
			"ua": "8900a/1.2 Mozilla/4.0 (compatible; MSIE 6.0; Windows CE; IEMobile 7.6)",
			"data": {
				"ie": 6,
				"mobile": "windows",
				"os": "windows"
			}
		}
	],
	"Mobile Firefox": [
		{
			"ua": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.1b2pre) Gecko/20081015 Fennec/1.0a1",
			"data": {
				"gecko": 1.91,
				"mobile": "gecko",
				"os": "windows"
			}
		},
		{
			"ua": "Mozilla/5.0 (Android; Linux armv7l; rv:9.0) Gecko/20111216 Firefox/9.0 Fennec/9.0",
			"data": {
				"gecko": 9,
				"mobile": "gecko",
				"os": "android"
			}
		},
		{
			"ua": "Mozilla/5.0 (X11; U; Linux armv7l; en-US; rv:1.9.2a1pre) Gecko/20090322 Fennec/1.0b2pre",
			"data": {
				"gecko": 1.92,
				"mobile": "gecko",
				"os": "linux"
			}
		}
	],
	"Mobile Safari": [
		{
			"ua": "Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_3 like Mac OS X; de-de) AppleWebKit/533.17.9 (KHTML, like Gecko)",
			"data": {
				"webkit": 533.179,
				"safari": 533.179,
				"mobile": "Apple",
				"iphone": 4.3,
				"ios": 4.3,
				"os": "ios"
			}
		},
		{
			"ua": "Mozilla/5.0 (iPad; U; CPU OS 4_3_5 like Mac OS X; de-de) AppleWebKit/533.17.9 (KHTML, like Gecko)",
			"data": {
				"webkit": 533.179,
				"safari": 533.179,
				"mobile": "Apple",
				"ipad": 4.3,
				"ios": 4.3,
				"os": "ios"
			}
		},
		{
			"ua": "Mozilla/5.0 (iPad; U; CPU OS 4_2_1 like Mac OS X; ja-jp) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8C148 Safari/6533.18.5",
			"data": {
				"webkit": 533.179,
				"safari": 533.179,
				"mobile": "Apple",
				"ipad": 4.2,
				"ios": 4.2,
				"os": "ios"
			}
		},
		{
			"ua": "Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_0 like Mac OS X; en-us) AppleWebKit/532.9 (KHTML, like Gecko) Version/4.0.5 Mobile/8A293 Safari/6531.22.7",
			"data": {
				"webkit": 532.9,
				"safari": 532.9,
				"mobile": "Apple",
				"iphone": 4,
				"ios": 4,
				"os": "ios"
			}
		},
		{
			"ua": "Mozilla/5.0 (iPad; U; CPU OS 3_2 like Mac OS X; es-es) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B360 Safari/531.21.10",
			"data": {
				"webkit": 531.211,
				"safari": 531.211,
				"mobile": "Apple",
				"ipad": 3.2,
				"ios": 3.2,
				"os": "ios"
			}
		},
		{
			"ua": "Mozilla/5.0 (iPad; U; CPU OS 4_3_3 like Mac OS X; de-de) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8J2 Safari/6533.18.5",
			"data": {
				"webkit": 533.179,
				"safari": 533.179,
				"mobile": "Apple",
				"ipad": 4.3,
				"ios": 4.3,
				"os": "ios"
			}
		},
		{
			"ua": "NokiaC3-00/5.0 (04.60) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
			"data": {
				"webkit": 420,
				"safari": 420
			}
		}
	],
	"Opera Mini": [
		{
			"ua": "Opera/10.61 (J2ME/MIDP; Opera Mini/5.1.21219/19.999; en-US; rv:1.9.3a5) WebKit/534.5 Presto/2.6.30",
			"data": {
				"opera": 10.61,
				"mobile": "Opera Mini/5.1.21219/19.999"
			}
		},
		{
			"ua": "Opera/8.01 (J2ME/MIDP; Opera Mini/3.1.9427/1724; nb; U; ssr)",
			"data": {
				"opera": 8.01,
				"mobile": "Opera Mini/3.1.9427/1724"
			}
		},
		{
			"ua": "Opera/8.01 (J2ME/MIDP; Opera Mini/1.1.3475/hifi/tmobile/cz; SonyEricsson W880i; cs; U; ssr)",
			"data": {
				"opera": 8.01,
				"mobile": "Opera Mini/1.1.3475/hifi/tmobile/cz"
			}
		},
		{
			"ua": "Opera/9.80 (iPhone; Opera Mini/5.0.0176/764; U; en) Presto/2.4.15",
			"data": {
				"opera": 9.8,
				"mobile": "Opera Mini/5.0.0176/764"
			}
		},
		{
			"ua": "Opera/9.60 (J2ME/MIDP; Opera Mini/4.2.14320/554; U; cs) Presto/2.2.0",
			"data": {
				"opera": 9.6,
				"mobile": "Opera Mini/4.2.14320/554"
			}
		},
		{
			"ua": "Opera/9.60 (J2ME/MIDP; Opera Mini/5.0.15650/960; U; en) Presto/2.2.0",
			"data": {
				"opera": 9.6,
				"mobile": "Opera Mini/5.0.15650/960"
			}
		}
	],
	"Opera Mobile": [
		{
			"ua": "Opera/9.80 (S60; SymbOS; Opera Mobi/499; U; es-ES) Presto/2.4.18 Version/10.00",
			"data": {
				"opera": 10,
				"mobile": "opera",
				"os": "symbos"
			}
		},
		{
			"ua": "Opera/9.80 (Android 2.2; Opera Mobi/-2118645896; U; pl) Presto/2.7.60 Version/10.5",
			"data": {
				"opera": 10.5,
				"mobile": "opera",
				"os": "android"
			}
		},
		{
			"ua": "Opera/9.80 (Windows Mobile; WCE; Opera Mobi/WMD-50430; U; en-GB) Presto/2.4.13 Version/10.00",
			"data": {
				"opera": 10,
				"mobile": "opera",
				"os": "windows"
			}
		},
		{
			"ua": "Opera/9.80 (Android; Linux; Opera Mobi/49; U; en) Presto/2.4.18 Version/10.00",
			"data": {
				"opera": 10,
				"mobile": "opera",
				"os": "android"
			}
		},
		{
			"ua": "Opera/9.80 (S60; SymbOS; Opera Mobi/352; U; de) Presto/2.4.15 Version/10.00",
			"data": {
				"opera": 10,
				"mobile": "opera",
				"os": "symbos"
			}
		},
		{
			"ua": "HTC_HD2_T9193 Opera/9.7 (Windows NT 5.1; U; en) V1.49.841.1 (71528)",
			"data": {
				"opera": 9.7,
				"os": "windows"
			}
		},
		{
			"ua": "SAMSUNG-GT-i8000/1.0 (Windows CE; Opera Mobi; U; en) Opera 9.5",
			"data": {
				"opera": 9.5,
				"mobile": "opera",
				"os": "windows"
			}
		}
	],
	"Palm Pre web browser": [
		{
			"ua": "Mozilla/5.0 (webOS/1.0; U; en-US) AppleWebKit/525.27.1 (KHTML, like Gecko) Version/1.0 Safari/525.27.1 Pre/1.0",
			"data": {
				"webkit": 525.271,
				"safari": 525.271,
				"mobile": "WebOS",
				"webos": 1
			}
		}
	]
};

});
