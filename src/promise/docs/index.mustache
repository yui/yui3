...
<p>
    ...
    a value of `undefined`. Sometimes that's ok, but it's often helpful to pass
    along _some_ data.
</p>
...
<p>
    The `onRejected` callback can then rethrow the error to propagate the
    ...
    `undefined` _which will mark the failure as recovered_, but with a resolved
...
<p>
    Because `showError` returned a value, and didn't rethrow the `reason`,
...
<p>
    ...
    simulataneously. This is where `Y.batch()` comes in.
</p>
