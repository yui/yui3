 ...
 * @requires widget, dd-plugin, resize-plugin, gallery-event-arrow, dd-constrain
...
ImageCropper = Y.Base.create('imagecropper', Y.Widget, [], {
	...
	*/
	...
	_bindResize: function (resizeKnob, contentBox) {
		resizeKnob.plug(Y.Plugin.Resize);
		var resize = resizeKnob.resize;
	...
	_bindDrag: function (resizeKnob, contentBox) {
		resizeKnob.plug(Y.Plugin.Drag, {
			handles: [this.get('resizeMask')]
		});
		var drag = resizeKnob.dd;
	...
	initializer: function () {
		this.setAttrs({
			...
			initWidth: this.get('initWidth'),
			initHeight: this.get('initHeight')
		});
	...
	destructor: function () {
		this.get('resizeKnob').unplug([Y.Plugin.Drag, Y.Plugin.Resize]);
	...
	ATTRS: {
		 ...
		 * True when either resizing or dragging the `resizeKnob`
		 *
		 * @attribute status
		 * @type {Boolean}
		 * @readOnly
