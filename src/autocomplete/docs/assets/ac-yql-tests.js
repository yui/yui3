YUI.add('ac-yql-tests', function(Y) {

    var suite = new Y.Test.Suite('ac-yql example test suite'),
        Assert = Y.Assert;
    ...
    suite.add(new Y.Test.Case({
        ...
        'test type selecting 3rd item from ac list' : function() {
            var input =  Y.one('.example #demo #ac-input'),
                ...
                Assert.areNotEqual('ca', input.get('value'), ' - Failed to put selected item text in input')
