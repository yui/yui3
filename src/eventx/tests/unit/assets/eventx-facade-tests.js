YUI.add("eventx-facade-tests", function(Y) {
    ...
    suite.add(new Y.Test.Case({
        ...
        _should: {
            ignore : {
                ...
                // This test belongs elsewhere
                test_node_publish: true,

                // These tests test features that aren't in the core eventx
                // module. Maybe they should required by the compat module?
                testPrefix: true,
                testDetachKey: true,
                testDetachAllByKey: true,
                testChain: true,
                testBroadcast: true,
        ...
        testAugment: function() {
            ...
            handle = o.on('testAugment', function(e, arg1, arg2) {
                Y.Assert.isInstanceOf(O, this);
                Y.Assert.isInstanceOf(Y.EventFacade, e);
                Y.Assert.areSame('afoo', e.foo);
                Y.Assert.areSame(1, e.details[1]);
                Y.Assert.areSame(1, arg1);
                Y.Assert.areSame(2, arg2);
                fired = true;
            });
        ...
        testDetachKey: function() {
            ...
            target.on('handle|test:event', function() {
                fired1 = true;
            });

            // one listener
            target.fire('test:event');
            Y.Assert.isTrue(fired1);
            Y.Assert.isFalse(fired2);

            target.detach('handle|test:event');
            ...
            target.after('handle|test:event', function(arg1) {
            ...
            // comma or pipe
            target.on('handle|test:event', function(arg1) {
                Y.Assert.areEqual('orange', arg1);
                Y.Assert.isFalse(fired2);
                fired1 = true;
            });
            ...
            var ret = target.detach('handle|test:event');
        ...
        testDetachAllByKey: function() {
            ...
            target.after('handle|event2', function() {
                fired2 = true;
            });

            target.on('handle|event2', function() {
                fired1 = true;
            });
        ...
        testBubble: function() {
            ...
            // eventx target.fire() is chainable
            //Y.Assert.isTrue(ret);
        ...
        testDetachHandle: function() {
            ...
            // This was invalidated when changing from Y to target, since it's
            // testing that Y.detachAll() does nothing. That is ok in eventx.
            /*
            var handle3 = target.on('y:click', function() {
                count++;
            });
            ...
            //Y.detachAll();
            // Note: It can with eventx. It unsubscribes custom events only.
            // It's still not likely a good idea, but then neither is
            // publishing and subscribing events on Y
            target.detachAll();
        ...
        test_fire_once: function() {
            ...
            target.publish('fireonce', {
                fireOnce: true
            });

            target.fire('fireonce', 'foo', 'bar');

            target.on('fireonce', function(arg1, arg2) {
                notified++;
            ...
            target.fire('fireonce', 'foo2', 'bar2');
            target.fire('fireonce', 'foo3', 'bar3');
