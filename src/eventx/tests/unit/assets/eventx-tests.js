YUI.add('eventx-tests', function(Y) {
...
baseSuite.add(new Y.Test.Case({
    ...
    "test new Y.EventTarget(config)": function () {
        var target1 = new Y.EventTarget(),
            ...
            default1 = target1.getEvent('@default'),
            default2 = target2.getEvent('@default');

        //Y.Assert.isString(config1.id);
        Y.Assert.isObject(target1._yuievt.events);
        Y.Assert.isObject(target2._yuievt.events);
        Y.Assert.isObject(target1._yuievt.subs);
        Y.Assert.isObject(target2._yuievt.subs);
        ...
        Y.Assert.isInstanceOf(Y.CustomEvent, default1);
        Y.Assert.isInstanceOf(Y.CustomEvent, default2);

        // These aren't valuable assertions because they only test that
        // instantiation configs are mixed onto the default event. They don't
        // test behavior. And behavior is derived differently, anyway.
        Y.Assert.isUndefined(default1.broadcast);
        Y.Assert.isTrue(default1.bubbles);
        //Y.Assert.areSame(target1, default2.context);
        Y.Assert.isUndefined(default1.defaultTargetOnly);
        Y.Assert.isUndefined(default1.emitFacade);
        Y.Assert.isUndefined(default1.fireOnce);
        Y.Assert.isUndefined(default1.monitored);
        Y.Assert.isUndefined(default1.queuable);

        //Y.Assert.isObject(config2.defaults);
        Y.Assert.areSame(2, default2.broadcast);
        Y.Assert.isFalse(default2.bubbles);
        Y.Assert.areSame(target1, default2.context);
        Y.Assert.isTrue(default2.defaultTargetOnly);
        Y.Assert.isTrue(default2.emitFacade);
        Y.Assert.isTrue(default2.fireOnce);
        Y.Assert.isTrue(default2.monitored);
        Y.Assert.isTrue(default2.queuable);
        //Y.Assert.isUndefined(default2.async);
    },
...
baseSuite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            ...
            // subscribe doesn't publish. It's handled by the default event.
            "test auto-publish on subscribe": true,

            // No reasonable use case, since on([type], fn)
            "test target.on({ type: true }, fn)": true,

            // Unreasonable use case support. I'm ok breaking these.
            "test target.on({ type: { fn: wins } }, fn)": true,
            "test target.on({ type: { fn: wins } }, fn, obj, args)": true,
            "test target.on({ type: { fn: wins, context: wins } }, fn, ctx, args)": true,
            "test target.on({ type: { context: wins } }, callback, ctx, args)": true
        }
    },
    ...
    "test target.on(type, fn)": function () {
        var target = new Y.EventTarget(),
            ...
            subscribers, afters,
            subscription, thisObj, fired, argCount, testEvent;
        ...
        subscription = target.on("test", callback);

        //testEvent = events.test;

        Y.Assert.isObject(subs.test);
        subscribers = subs.test.on;
        afters      = subs.test.after;
        ...
        Y.Assert.isUndefined(afters);
        //Y.Assert.areSame(1, testEvent.hasSubs());

        Y.Assert.isObject(subscription);
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, subscription);
        Y.Assert.areSame('test', subscription.type);
    ...
    "test target.on(type, fn) allows duplicate subs": function () {
        var target = new Y.EventTarget(),
            subs = target._yuievt.subs,
            count  = 0,
            subscribers, afters, testEvent, subscription1, subscription2;

        function callback() {
            count++;
        }

        subscription1 = target.on("test", callback);
        subscription2 = target.on("test", callback);
        ...
        Y.Assert.areNotSame(subscription1, subscription2);
        Y.Assert.areSame('test', subscription1.type);
        Y.Assert.areSame(subscription1.type, subscription2.type);
    ...
    "test target.on([type], fn)": function () {
        ...
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame('test', handle.subs[0].type);
        Y.Assert.isUndefined(handle.type);
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle.subs[0]);
    ...
    "test target.on([typeA, typeB], fn)": function () {
        ...
        subscribers = subs.test1.on;
        afters      = subs.test1.after;
        ...
        subscribers = subs.test2.on;
        afters      = subs.test2.after;
        ...
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame('test1', handle.subs[0].type);
        Y.Assert.areSame('test2', handle.subs[1].type);
    ...
    "test target.on([typeA, typeA], fn)": function () {
        ...
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame('test', handle.subs[0].type);
        Y.Assert.areSame('test', handle.subs[1].type);
        Y.Assert.isUndefined(handle.type);
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle.subs[0]);
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle.subs[1]);
    ...
    "test target.on([], fn) does nothing": function () {
        ...
        Y.Assert.areSame(0, Y.Object.keys(subs).length);

        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame(0, handle.subs.length);
        Y.Assert.isUndefined(handle.type);
    },

    "test target.on([{ fn: fn, context: obj }]) does nothing": function () {
        var target = new Y.EventTarget(),
            events = target._yuievt.subs,
            count = 0,
            handle, name, subs, i;
    ...
    "test target.on({ type: fn })": function () {
        ...
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame(1, handle.subs.length);
        Y.Assert.areSame('test1', handle.subs[0].type);
        Y.Assert.isUndefined(handle.type);
        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle.subs[0]);
        ...
        Y.Assert.areSame(1, subs.test2.on.length);
        Y.Assert.areSame(1, subs.test3.on.length);

        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.isArray(handle.subs);
        Y.Assert.areSame(2, handle.subs.length);
        Y.Assert.areSame('test2', handle.subs[0].type);
        Y.Assert.areSame('test3', handle.subs[1].type);
        Y.Assert.isUndefined(handle.type);
    },

    "test target.on({ type: true }, fn)": function () {
        // TODO? test not updated, because it doesn't represent a reasonable
        // use case, so far as I can tell. on([type1,type2], fn) handles this.
        // Moved to _should.ignore
        var target = new Y.EventTarget(),
            events = target._yuievt.events,
            subscribers,
            afters,
        ...
        subscribers = testEvent._subscribers;
        afters = testEvent._afters;
    ...
    "test callback context": function () {
        ...
        // Commented out because the use case is unreasonable for the code cost
        /*
        target.on({
            "test10": { fn: isTarget },
    ...
    "test subscription bound args": function () {
        ...
        // Commmented out because the use case is unreasonable for the code cost
        /*
        target.on({
            "test6": true,
            "test7": false
    ...
    "test target.on('click', fn) registers custom event only": function () {
        ...
        event = target.getEvent('click');

        // This assertion is useless because DOMEvent is a CustomEvent subclass
        Y.Assert.isInstanceOf(Y.CustomEvent, event);
        if (Y.Event && Y.Event.DOMEvent) {
            Y.Assert.isNotInstanceOf(Y.Event.DOMEvent, event);
        }
        Y.Assert.areSame(1, subs.click.on.length);
    }
...
baseSuite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            ...
            // No reasonable use case, since after([type], fn)
            "test target.after({ type: true }, fn)": true,

            // Unreasonable use case support. I'm ok breaking these.
            "test target.after({ type: { fn: wins } }, fn)": true,
            "test target.after({ type: { fn: wins } }, fn, obj, args)": true,
            "test target.after({ type: { fn: wins, context: wins } }, fn, ctx, args)": true,
            "test target.after({ type: { context: wins } }, callback, ctx, args)": true
        }
    },
    ...
    "test target.after(type, fn)": function () {
        ...
        Y.Assert.isUndefined(subscribers);
        Y.Assert.areSame(1, afters.length);
        //Y.Assert.areSame(1, testEvent.hasSubs());

        Y.Assert.isInstanceOf(Y.CustomEvent.Subscription, handle);
        Y.Assert.areSame('test', handle.type);
        //Y.Assert.isInstanceOf(Y.Subscriber, handle.sub);
    ...
    "test target.after(type, fn) allows duplicate subs": function () {
        ...
        Y.Assert.areSame('test', handle1.type);
        Y.Assert.areSame('test', handle2.type);
        //Y.Assert.areNotSame(handle1.sub, handle2.sub);
    ...
    "test target.after({ type: fn })": function () {
        ...
        //Y.Assert.isInstanceOf(Y.CustomEvent, events.test2);
        //Y.Assert.isInstanceOf(Y.CustomEvent, events.test3);
        Y.Assert.areSame(1, subs.test2.after.length);
        Y.Assert.areSame(1, subs.test3.after.length);
    ...
    "test target.after({ type: true }, fn)": function () {
        // Not updated because the test is ignored
        var target = new Y.EventTarget(),
            events = target._yuievt.events,
            subscribers,
            afters,
    ...
    "test target.after('click', fn) registers custom event only": function () {
        ...
        Y.Assert.areSame(1, subs.click.after.length);
    }
...
baseSuite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            ...
            // No reasonable use case, since after([type], fn)
            "test target.once({ type: true }, fn)": true
        }
    },

    "test auto-publish on subscribe": function () {
        // Not updated because the test doesn't apply
        var target = new Y.EventTarget(),
            events = target._yuievt.events,
            publishCalled;
    ...
    "test target.once([typeA, typeB], fn)": function () {
        ...
        Y.Assert.areSame(1, count);
        Y.Assert.areSame(target, thisObj);
        Y.Assert.isNull(subs.test1);

        target.fire("test2");

        Y.Assert.areSame(2, count);
        Y.Assert.areSame(target, thisObj);
        Y.Assert.isNull(subs.test2);
    },
    ...
    "test target.once({ type: true }, fn)": function () {
        ...
        Y.Assert.areSame(0, events.test2._subscribers.length);
        Y.Assert.areSame(1, events.test3._subscribers.length);

        target.fire("test3");

        Y.Assert.areSame(0, events.test2._subscribers.length);
        Y.Assert.areSame(0, events.test3._subscribers.length);
    },
    ...
    "test callback context": function () {
        ...
        */
        ...
        Y.Assert.isNull(subs.test2);
        Y.Assert.isNull(subs.test3);
        Y.Assert.isNull(subs.test4);
        Y.Assert.isNull(subs.test5);
        Y.Assert.isNull(subs['prefix:test6']);
        Y.Assert.isNull(subs.test7);
        Y.Assert.isNull(subs['prefix:test8']);
        Y.Assert.isNull(subs.test9);
        /*
        Y.Assert.areSame(0, events.test10._subscribers.length);
        Y.Assert.areSame(0, events.test11._subscribers.length);
        Y.Assert.areSame(0, events.test12._subscribers.length);
        Y.Assert.areSame(0, events['prefix:test13']._subscribers.length);
        */
    },
    ...
    "test subscription bound args": function () {
        ...
        */
        ...
        Y.Assert.isNull(subs.test4);
        Y.Assert.isNull(subs.test5);
        //Y.Assert.areSame(0, events.test6._subscribers.length);
        //Y.Assert.areSame(0, events.test7._subscribers.length);
    },
...
baseSuite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            ...
            // No reasonable use case, since after([type], fn)
            "test target.onceAfter({ type: true }, fn)": true
        }
    },
    ...
    "test auto-publish on subscribe": function () {
        // Not updated because test is ignored
        var target = new Y.EventTarget(),
            events = target._yuievt.events,
            publishCalled;
    ...
    "test target.onceAfter([typeA, typeB], fn)": function () {
        ...
        //Y.Assert.isInstanceOf(Y.CustomEvent, testEvent1);
        Y.Assert.isUndefined(subs.test1.on);
        Y.Assert.isArray(subs.test1.after);
        Y.Assert.areSame(1, subs.test1.after.length);
        //Y.Assert.areSame(1, testEvent1.hasSubs());

        //testEvent2 = events.test2;

        //Y.Assert.isInstanceOf(Y.CustomEvent, testEvent2);
        Y.Assert.isUndefined(subs.test2.on);
        Y.Assert.isArray(subs.test2.after);
        Y.Assert.areSame(1, subs.test2.after.length);
        //Y.Assert.areSame(1, testEvent2.hasSubs());
...
baseSuite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            "test target.on('cat|__', fn) + target.detach('cat|___')": true,
            "test target.on('cat|__', fn) + target.detach('cat|___', fn)": true,
            "test target.on('cat|__', fn) + target.detach('cat|*')": true,
            "test target.on({'cat|type': fn}) + target.detach(type, fn)": true,
            "test target.on({'cat|type': fn}) + target.detach('cat|type')": true,
            "test target.on({'cat|type': fn}) + target.detach('cat|*')": true,
            "test target.after('cat|__', fn) + target.detach('cat|___')": true,
            "test target.after('cat|__', fn) + target.detach('cat|___', fn)": true,
            "test target.after('cat|__', fn) + target.detach('cat|*')": true,
            "test target.after({'cat|type': fn}) + target.detach(type, fn)": true,
            "test target.after({'cat|type': fn}) + target.detach('cat|type')": true,
            "test target.after({'cat|type': fn}) + target.detach('cat|*')": true,
            "test target.on(type) + target.detach(prefix:type)": true,
            "test target.after(type) + target.detach(prefix:type)": true
        }
    },
