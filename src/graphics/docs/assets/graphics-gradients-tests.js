YUI.add('graphics-gradients-tests', function(Y) {
    var suite = new Y.Test.Suite('graphics-gradients-tests example test suite'),
    ...
    suite.add(new Y.Test.Case({
        ...
        testGraphicsLoaded : function()
        {
            ...
            Y.Assert.areEqual(2, shapes.size(), "There should be 2 shapes.");
        },
        ...
        testRect: function()
        {
            ...
            if(ENGINE != "canvas")
            {
                fill = Y.ShapeNode.one(rect).getFill();
                stops = fill.stops;
                for(; i < len; i = i + 1)
                {
                    stop = stops[i];
        ...
        testCircle: function()
        {
            var circle = Y.all(CIRCLE).shift(),
                circleWidth = 100,
                circleHeight = 100,
                circleDimensions = Y.ShapeNode.one(circle).getDimensions("circle"),
                defaultStops = [
                    {color: "#ff6666", offset: 0, opacity: 1},
                    {color: "#00ffff", offset: 0.4, opacity: 1},
                    {color: "#000000", offset: 0.7, opacity: 1}
                ],
                defaultStop,
                i = 0,
                len = defaultStops.length,
            ...
            if(ENGINE != "canvas")
            {
                fill = Y.ShapeNode.one(circle).getFill();
                stops = fill.stops;
                for(; i < len; i = i + 1)
                {
                    stop = stops[i];
