YUI.add('graphics-violin-tests', function(Y) {
    var suite = new Y.Test.Suite('graphics-violin-tests example test suite'),
    ...
    IMPLEMENTATION = {
        ...
        vml: {
            getStroke: function()
            {
                ...
                opacity = strokeNode ? strokeNode.getAttribute("opacity") : 1;
            ...
            getFill: function()
            {
                ...
                opacity = fillNode ? fillNode.getAttribute("opacity") : 1;
                if(color.value)
                {
                    color = color.value;
                }
    ...
    suite.add(new Y.Test.Case({
        ...
        testGraphicsLoaded : function()
        {
            var shapes = Y.all(SHAPE),
                ...
                paths = Y.all(PATH),
                ie = Y.UA.ie,
                numberofpaths = ie && ie < 9 ? 19 : 20,
                totalshapes = numberofpaths + 8;
            Y.Assert.areEqual(totalshapes, shapes.size(), "There should be " + totalshapes + " shape.");
            Y.Assert.areEqual(7, ellipses.size(), "There should be 7 Ellipse instance.");
            Y.Assert.areEqual(1, rects.size(), "There should be 3 Rect instances.");
            Y.Assert.areEqual(numberofpaths, paths.size(), "There should be " + numberofpaths + " Path instances.");
        }
    }));
