...
function initSynth() {
    Y.Event.define('synth', {
        ...
        // define({ on: fnA, delegate: fnB }) is moved to _oldOn and
        // _oldDelegate, called from SyntheticEvent.on. If either calls this.on
        // it will infinitely recurse. This should affect few implementations,
        // and fixing it should be easy, so I'm not going to sweat it.
        /*
        delegate: function () {
...
function undefineSynth() {
    delete Y.Event.DOM_EVENTS.synth;
...
suite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            "Y.Event.define([name1, name2]) should add to DOM_EVENTS": true,
            "Y.Event.define({ type: ...}) should register a new synth in DOM_EVENTS": true,
            "Y.Event.define({...}, true) should overwrite existing synth": true
        }
    },

    "Y.Event.define(name) should add to DOM_EVENTS": function () {
        delete Y.Event.DOM_EVENTS.mouseover;
        
        Y.Assert.isUndefined(Y.Event.DOM_EVENTS.mouseover);

        Y.Event.define('mouseover');

        Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.mouseover);
    },

    "Y.Event.define([name1, name2]) should add to DOM_EVENTS": function () {
        delete Y.Event.DOM_EVENTS.mouseover;
        delete Y.Event.DOM_EVENTS.mouseout;
        
        Y.Assert.isUndefined(Y.Event.DOM_EVENTS.mouseover);
        Y.Assert.isUndefined(Y.Event.DOM_EVENTS.mouseout);

        Y.Event.define(['mouseover', 'mouseout']);

        Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.mouseover);
        Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.mouseout);
    },
    ...
    "Y.Event.define(type, {...}) should register a new synth in DOM_EVENTS": function () {
        ...
        Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.synth);
        //Y.Assert.isNotUndefined(Y.Env.evt.plugins.synth);
        //Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.synth.eventDef);
        //areSame(0, Y.Event.DOM_EVENTS.synth.eventDef.index);
        areSame(0, Y.Event.DOM_EVENTS.synth.index);
    },
    ...
    "Y.Event.define(..., true) should overwrite existing synth": function () {
        ...
        areSame(2, Y.Event.DOM_EVENTS.synth.index);
    },
    ...
    "Y.Event.define({ type: ...}) should register a new synth in DOM_EVENTS": function () {
        ...
        //Y.Assert.isNotUndefined(Y.Env.evt.plugins.synth);
        //Y.Assert.isNotUndefined(Y.Event.DOM_EVENTS.synth.eventDef);
        //areSame(Y.Event.DOM_EVENTS.synth, Y.Env.evt.plugins.synth);
        areSame(3, Y.Event.DOM_EVENTS.synth.eventDef.index);
    },
    ...
    "Y.Event.define({...}, true) should overwrite existing synth": function () {
        ...
        areSame(2, Y.Event.DOM_EVENTS.synth.eventDef.index);
    }
...
suite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            "test Y.on('synth', fn, notYetAvailable)": true,
            "test Y.on('synth', fn, notYetAvailable, thisObj)": true,
            "test Y.on('synth', fn, notYetAvailable, thisObj, arg)": true,
            "test Y.on('synth', fn, notYetAvailable, null, arg)": true
        },
    },
    ...
    "test Y.on('synth', fn) defaults to window": function (){
        ...
        areSame(Y.one(Y.config.win)._yuid, handle.details.nodeYuid);
...
suite.add(new Y.Test.Case({
    ...
    "test nodelist.on(x, fn)": function () {
        ...
        Y.ArrayAssert.itemsAreSame(
            ...
            currentTarget);
        // Changed nodelist.on() to default 'this' to the Node
    ...
    "test nodelist.on(x, fn, null, arg)": function () {
        ...
        Y.ArrayAssert.itemsAreSame(
            [Y.one('#p1_no'), Y.one('#p3_no'), Y.one('#inner_1_p1_no')],
            currentTarget);
        // Changed nodelist.on() to default 'this' to Node
        //Y.ArrayAssert.itemsAreSame([targets, targets, targets], thisObj);
        Y.ArrayAssert.itemsAreSame(
            [Y.one('#p1_no'), Y.one('#p3_no'), Y.one('#inner_1_p1_no')],
            currentTarget);
...
suite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            "test Y.delegate(synth, fn, notAvailableYet, filter)": true,
            "test Y.delegate(synth, fn, notAvailableYet, filter, thisObj)": true,
            "test Y.delegate(synth, fn, notAvailableYet, filter, thisObj, arg)": true,
            "test Y.delegate(synth, fn, notAvailableYet, filter, null, arg)": true
        },
    },
...
suite.add(new Y.Test.Case({
    ...
    _should: {
        fail: {
            // category not supported in core eventx
            "test node.on('cat|__', fn) + node.detach('cat|___')": true,
            "test node.on('cat|__', fn) + node.detach('cat|___', fn)": true,
            "test node.on('cat|__', fn) + node.detach('cat|*')": true,
            "test Y.on('cat|__', fn, sel) + node.detach('cat|___')": true,
            "test Y.on('cat|__', fn, sel) + node.detach('cat|___', fn)": true,
            "test Y.on('cat|__', fn) + node.detach('cat|*')": true,
...
suite.add(new Y.Test.Case({
    ...
    _should: {
        ignore: {
            // Y.on polling not supported by eventx
            "Y.on('synth', fn, '#not-here-yet', extra) should resubscribe with original arguments": true
        }
    },
...
}, '@VERSION@' ,{requires:['eventx-synthetic', 'test', 'node-event-simulate']});
