...
Y.Loader.prototype = {
    ...
    _attach: function(success) {
        ...
        var oldReqs = Y.merge(this.required),
            oldSorted = this.sorted.concat(),
            loader,
            attaching = this.attaching || this.sorted,
            ret = false, redo = false,
            keys;
        ...
        if (success) {
            ...
            loader.context = this;
            loader.calculate(null, this.loadType);
            // keys = Y.Object.keys(loader.required);
            keys = loader.sorted.concat();
        }

        if (keys && keys.length) {
            Y.log('new requirements: ' + keys);
            // this.required = loader.required;
            // this.sorted = loader.sorted;
            // this._loading = true;
            // this.attaching = attaching.concat(this.sorted);
            // this.loadNext();

            loader.attaching = loader.sorted.concat(attaching);

            _queue.running = false;
            loader.insert(null, this.loadType);
