YUI.add('profiler', function(Y) {
    ...
    function saveDataPoint(name, duration){
        var functionData        = createReport(name),
            callingFunctionData = createReport(stack[0]);

        // add duration to the caller's innerTime to facilitate ownTime calc
        if (callingFunctionData) {
            callingFunctionData.inner += duration;
        }

        functionData.calls++;
        functionData.points.push(duration);
        functionData.total += duration;
        functionData.own += duration - functionData.inner;
        functionData.inner = 0; // reset for the next call

        functionData.avg = functionData.total / functionData.calls;
        functionData.ownAvg = functionData.own / functionData.calls;
        functionData.min = functionData.calls === 1 ?
                                duration :
                                min(functionData.min, duration);
        functionData.max = max(functionData.max, duration);
    }
    ...
    Y.Profiler = {
         ...
         * @static
         */
        getFullReport : function (filter, sortBy, precision) {
            filter = filter || function(){return true;};

            var normalize = Y.Profiler.normalizeReportData,
                fullReport = {},
                total = 0,
                name;
            ...
            // Calculate total execution time for percentage time calculation
            for (name in report) {
                if (report.hasOwnProperty(name)) {
                    total += report[name].own;
                }
            }

            if (L.isFunction(filter)) {
                for (name in report){
                    if (report.hasOwnProperty(name) && filter(report[name])){
                        fullReport[name] = normalize(report[name], total, precision);
                    }
                }
            }
        ...
        normalizeReportData : function (data, total, precision) {
            return Y.merge(data, {
                min     : data.min.toFixed(precision),
                max     : data.max.toFixed(precision),
                total   : data.total.toFixed(precision),
                avg     : data.avg.toFixed(precision),
                own     : data.own.toFixed(precision),
                ownAvg  : data.ownAvg.toFixed(precision),
                inner   : data.inner.toFixed(precision),
                percent : total ? ((data.own / total) * 100).toFixed(precision) : 100
            });
        },
