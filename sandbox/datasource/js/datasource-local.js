...
Y.extend(DSLocal, Y.Base, {
    ...
    */
    ...
    _defStartFn : function (e) {
        Y.log("Transaction " + e.transaction.id + " started.","info",this.toString());
    ...
    _defRequestFn: function(e) {
        ...
        if (e.error) {
            ...
            // worth using Y.merge(e,{data:data}) to preserve pre-data
            // state for after request listeners?
            e.data = data;
    ...
    _defResponseFn: function(e) {
        ...
        Y.log("Transaction " + e.transaction.id + " complete. Request: " + Y.dump(e.request) + " . Response: " + Y.dump(e.response), "info", this.toString());
    ...
    _defSuccessFn : function (e) {
        ...
        Y.log("Transaction  " + e.transaction.id + " completed successfully.", "info", this.toString());
    },
    ...
    _defFailureFn : function (e) {
        e.transaction.fire("end",e);
        Y.log("Transaction " + e.transaction.id + " failed.", "info", this.toString());
    },
    ...
    _defErrorFn : function (e) {
        if (e.error) {
            Y.log("Error in request: "+e.error.message,"error",this.toString());
            e.transaction.fire("end",e);
        }
    },

    _defEndFn : function (e) {
        Y.log("Transaction " + e.transaction.id + " completed","info",this.toString());
    },
    ...
    sendRequest: function(request, callback, payload) {
        ...
        Y.log("Transaction " + t.id + " sent request: " + Y.dump(request), "info", this.toString());
