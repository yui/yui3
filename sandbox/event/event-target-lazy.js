...
ET.prototype = {
    ...
    on: function(type, fn, context) {
        ...
        var parts = _parseType(this._yuievt.config.prefix, type), f, c, args, ret, ce,
            detachcategory, handle, store = Y.Env.evt.handles, after, adapt, opts;
        ...
        if (!handle) {
            ...
            if (!ce) {
                opts = this._yuievt.pending[type] || null;
                ce = this._publish(type, opts);
            }
...
// ce.log("publish applying config to published event: '"+type+"' exists", 'info', 'event');
    ...
    publish: function(type, opts) {
        ...
        if (L.isObject(type)) {
            Y.each(type, function(v, k) {
                this.publish(k, v || opts); 
            }, this);
            return;
        }

        this._yuievt.pending[type] = opts;
    },
    ...
    fire: function(type) {
        ...
        if (!ce) {
            ...
            if (this._yuievt.hasTargets) {
                // switched to a lazy publishing scheme for performance reasons,
                // if the event is fired w
                opts = this._yuievt.pending[type] || null;
                ce = this._publish(t, opts);
