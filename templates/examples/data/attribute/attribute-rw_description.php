...
<p>We first attempt to set values for both attributes in the contructor (used to intialize the attributes) and see that only <code>bar</code>, the <code>writeOnce</code> attribute, gets set to the user provided value:</p>
...
<p>We then attempt to set values for both attributes using <code>set</code>, and see that niether of the values change:</p>
...
<p>Although the user cannot update the value of <code>readOnly</code> attributes, it maybe neccessary for the host object to update it's value interally. The example shows how this can be done, using the private <code>_conf</code> state property on the host:</p>
