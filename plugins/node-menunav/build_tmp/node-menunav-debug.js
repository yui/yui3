	...
	if (oRootMenu) {
		...
		if (bUseARIA) {
			...
			oLIs = oRootMenu.queryAll("li").each(function (node) {
				...
				var oItem = node.query(FIRST_CHILD_SELECTOR);
			
				oItem.set(TAB_INDEX, -1);
				setARIARole(oItem, MENUITEM);
			...
			oRootMenu.queryAll((PERIOD + CSS_MENU_LABEL)).each(function (node) {
				...
				setARIAProperty(node, "haspopup", TRUE);
			...
			if (oLIs) {
				...
				oFirstItem = oLIs.item(0).query(FIRST_CHILD_SELECTOR);

				oFirstItem.set(TAB_INDEX, 0);
...
MenuNav.prototype = {
	...
	_onDocFocus: function (event) {
		...
		if (menuNav._hasFocus) {
			...
			if (bUseARIA) {
				menuNav._firstItem.set(TAB_INDEX, 0);
			}
